...
# Step79：原版 GameCamera.getZoom()/getScreenRect() 默认返回目标值（_zoom / _noTweenRect），
# 不是缓动后的显示值（_stageScale / _rect）。地图边缘限制必须用 target camera。
var camera_target_zoom: float = 1.0
var camera_target_logic_zoom: float = 1.0
var camera_target_rect: Rect2 = Rect2(Vector2.ZERO, Vector2(800, 600))
...
# Step80：原版 EffectCtrl.bisha(isSuper=true) 会 cameraFocusOne(target)，endBisha() 会 cameraResume()。
var original_bisha_super_camera_focus_active: bool = false
...
# Step78：原版 GameConfig.X_SIDE_OFFSET = 10。角色逻辑脚底点被限制在 line_left+10 / line_right-10；
# 之前的 42 是为了视觉边缘内缩的临时值，不符合原版 GameLogic.fixGameSpritePosition。
const ORIGINAL_MAP_X_SIDE_OFFSET: float = 10.0
...
const GODOT_XIANSHI_CAMERA_VISUAL_SCALE: float = 1.00
...
# 原版 EffectCtrl.doHitEffect(hitvo, hitRect) 直接使用 hitRect.center。
# 当前 Godot 的 hitRect 是按 Node2D/PNG centered 坐标推出来的，需要补一个“Flash 注册点 -> Godot 可见角色”的视觉 Y 偏移。
# Step82 把所有 common hit 都补到脚底 offset，J/连续J 的刀击会压得过低。
# Step83：普通 J / 连续 J / 空中 J 单独按角色可见高度约 2/3 位置补偿，落在胸腹/刀击交汇高度附近。
const ORIGINAL_HIT_EFFECT_VISUAL_Y_CORRECTION: float = ORIGINAL_FIGHTER_FOOT_OFFSET_Y
const ORIGINAL_AIZEN_J_HIT_EFFECT_VISUAL_Y_RATE: float = 0.66
const ORIGINAL_AIZEN_J_HIT_EFFECT_VISUAL_Y_CORRECTION: float = ORIGINAL_FIGHTER_FOOT_OFFSET_Y * ORIGINAL_AIZEN_J_HIT_EFFECT_VISUAL_Y_RATE
const ORIGINAL_AIZEN_J_HIT_IDS: Array[String] = ["k1", "k2", "k3", "k4", "k5", "k52", "tk"]
...
# Step60：按原版 ConfigVO.as 的 P2 默认键位接入 2P 操作：
# ↑/↓/←/→ + Num1/Num2/Num3/Num4/Num5/Num6。Num6 对应 O/beckons，当前仍暂不接支援/换人。
var prev_p2_attack_pressed: bool = false
var prev_p2_jump_pressed: bool = false
var prev_p2_dash_pressed: bool = false
var prev_p2_skill_pressed: bool = false
var prev_p2_bisha_pressed: bool = false

# Step22-24 / Step82：输入响应优化。
# 原版 FighterKeyCtrl.initlize(): _justDown=2；GameInputer.JUST_DOWN_DELAY=0.1 秒。
# 也就是攻击/跳跃/瞬步/技能/必杀的 justDown 会保留约 3 帧，
# 时间轴 setAttack / setSkill2 窗口出现时可立即被 renderFloorAction/renderAirAction 消化。
const ORIGINAL_INPUT_BUFFER_TIME: float = 3.0 / 30.0
...
var p2_attack_buffer_timer: float = 0.0
var p2_skill_buffer_timer: float = 0.0
var p2_bisha_buffer_timer: float = 0.0
var p2_jump_buffer_timer: float = 0.0
var p2_dash_buffer_timer: float = 0.0
...
var active_attacker_start_ms: int = 0
...
var original_move_target_actor: Variant = null
...
var naruto_hit_vo_stage1: Dictionary = {}
var character_attack_manifests: Dictionary = {}
var character_attacker_manifests: Dictionary = {}


func load_naruto_stage1_hit_vo() -> void:
	# Stage90-95：兼容旧 naruto_hitvo_stage1.json，同时优先读取正式 naruto_hitvo.json。
	var candidates: Array[String] = [
		"res://assets/characters/naruto/naruto_hitvo.json",
		"res://assets/characters/naruto/naruto_hitvo_stage1.json"
	]
	for path in candidates:
		...
		if typeof(data) == TYPE_DICTIONARY:
			naruto_hit_vo_stage1 = data
			print("Loaded Naruto HitVO: ", path, " count=", naruto_hit_vo_stage1.size())
			return


func load_character_attack_manifest(char_id: String) -> void:
	var id: String = char_id.to_lower()
	if character_attack_manifests.has(id):
		return
	var path: String = "res://assets/characters/" + id + "/" + id + "_attack_manifest.json"
	if not FileAccess.file_exists(path):
		character_attack_manifests[id] = {}
		return
	var f := FileAccess.open(path, FileAccess.READ)
	if f == null:
		character_attack_manifests[id] = {}
	...
	var data: Variant = JSON.parse_string(f.get_as_text())
	if typeof(data) == TYPE_DICTIONARY:
		character_attack_manifests[id] = data
		var actions: Dictionary = data.get("actions", {}) as Dictionary
		print("Loaded attack manifest: ", id, " actions=", actions.size())
	else:
		character_attack_manifests[id] = {}


func load_character_attacker_manifest(char_id: String) -> void:
	var id: String = char_id.to_lower()
	if character_attacker_manifests.has(id):
		return
	var path: String = "res://assets/characters/" + id + "/" + id + "_attacker_manifest.json"
	if not FileAccess.file_exists(path):
		character_attacker_manifests[id] = {}
		return
	var f := FileAccess.open(path, FileAccess.READ)
	if f == null:
		character_attacker_manifests[id] = {}
	...
	var data: Variant = JSON.parse_string(f.get_as_text())
	if typeof(data) == TYPE_DICTIONARY:
		character_attacker_manifests[id] = data
		var attackers: Dictionary = data.get("attackers", {}) as Dictionary
		print("Loaded attacker manifest: ", id, " attackers=", attackers.size())
	else:
		character_attacker_manifests[id] = {}


func load_all_character_stage90_data() -> void:
	load_naruto_stage1_hit_vo()
	load_character_attack_manifest("naruto")
	load_character_attacker_manifest("naruto")


func get_hit_vo_for_attacker(attacker: Variant, hit_vo_id: String) -> Dictionary:
	var char_id: String = get_fighter_character_id(attacker, "aizen")
	if char_id == "naruto" and naruto_hit_vo_stage1.has(hit_vo_id):
		return naruto_hit_vo_stage1.get(hit_vo_id, {}) as Dictionary
	return AIZEN_HIT_VO.get(hit_vo_id, {}) as Dictionary
...
# Step111：BattlePlaceholder 自身使用的原版动画 FPS。ManifestFighter 里也有同名概念，但脚本作用域不共享。
...
const ORIGINAL_NARUTO_PROJECTILE_REHIT_FRAMES := 4.0
const ORIGINAL_NARUTO_JUMP_ATTACK_REACH_PAD_X := 0.0 # Step130: 原版 tkatm 全矩阵重算后不再额外扩展
const ORIGINAL_NARUTO_JUMP_ATTACK_REACH_PAD_Y := 0.0 # Step130: 原版 tkatm 全矩阵重算后不再额外扩展
...
# Step81：按原版 EffectModel.as 统一还原所有命中特效反馈参数。
# 原版 EffectCtrl.doEffectVO 会在播放 EffectVO 后统一处理 freeze / shake / shine；
# 之前 Godot 只对电击和少数重击做近似，导致 J/连续J、技能、必杀的 hit stop 与震屏偏差。
# 这里不改变 HitVO、攻击面、伤害、怒气，只补齐视觉/反馈层。
const ORIGINAL_HIT_EFFECT_META := {
	1: {"freeze": 0.05, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0},
	2: {"freeze": 0.05, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0},
	3: {"freeze": 0.40, "shine_color": Color(1, 1, 1, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	4: {"freeze": 0.05, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0},
	5: {"freeze": 0.40, "shine_color": Color(1, 1, 1, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	6: {"freeze": 0.40, "shine_color": Color(1, 1, 1, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	7: {"freeze": 0.40, "shine_color": Color(1.0, 1.0, 0.4039, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	8: {"freeze": 0.40, "shine_color": Color(0.6392, 0.8980, 0.9608, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	9: {"freeze": 0.40, "shine_color": Color(0.5098, 0.5333, 0.8235, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	11: {"freeze": 0.40, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0}
}

const ORIGINAL_DEFENSE_EFFECT_META := {
	1: {"freeze": 0.05, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0},
	2: {"freeze": 0.05, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0},
	3: {"freeze": 0.40, "shine_color": Color(1, 1, 1, 1), "shine_alpha": 0.10, "shake_pow": 6.0, "shake_time": 0.40},
	4: {"freeze": 0.05, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0},
	5: {"freeze": 0.40, "shine_color": Color(1, 1, 1, 1), "shine_alpha": 0.15, "shake_pow": 6.0, "shake_time": 0.40},
	6: {"freeze": 0.40, "shine_color": Color(1, 1, 1, 1), "shine_alpha": 0.10, "shake_pow": 6.0, "shake_time": 0.40},
	7: {"freeze": 0.40, "shine_color": Color(1.0, 1.0, 0.4039, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	8: {"freeze": 0.40, "shine_color": Color(0.6392, 0.8980, 0.9608, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	9: {"freeze": 0.40, "shine_color": Color(0.5098, 0.5333, 0.8235, 1), "shine_alpha": 0.20, "shake_pow": 6.0, "shake_time": 0.40},
	11: {"freeze": 0.05, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_pow": 0.0, "shake_time": 0.0}
}

const ORIGINAL_STEEL_HIT_META := {"freeze": 0.40, "shine_color": Color(1, 1, 1, 1), "shine_alpha": 0.20, "shake_pow": 0.0, "shake_time": 0.0}
const ORIGINAL_BREAK_DEF_META := {"freeze": 0.50, "shine_color": Color(1, 1, 1, 0.0), "shine_alpha": 0.0, "shake_x": 6.0, "shake_y": 0.0, "shake_time": 0.40}
...
# Step64：原版 GameStartCtrler / GameEndCtrler 审计。
# 开场：cameraFocusOne(P1) -> P1.sayIntro() -> cameraFocusOne(P2) -> P2.sayIntro() -> cameraResume() -> showStart(Round/Fight)。
# 结算：playKO/showEnd 等待败者进入 DEAD/倒地流程后，cameraFocusOne(winner) / showWinner，随后 GameEndCtrler 调 winner.win() 并保持约 3 秒。
const ORIGINAL_CAMERA_FOCUS_ONE_ZOOM: float = 3.5
const ORIGINAL_START_FOCUS_HOLD_TIME: float = 0.5
const ORIGINAL_START_AFTER_INTRO_HOLD_TIME: float = 0.3
const ORIGINAL_START_RESUME_HOLD_TIME: float = 0.1
const ORIGINAL_WINNER_FOCUS_HOLD_TIME: float = 0.5
const ORIGINAL_WIN_ACTION_HOLD_TIME: float = 33.0 / 30.0 # Aizen Step75：胜利动作停在 win/0033.png 后立即弹出结算菜单。Naruto Step131 会按动作帧数计算。
# Step66：原版 cameraFocusOne(display) 聚焦的是 Flash 显示对象；当前 ManifestFighter 使用 centered PNG，
# Node2D.position 不是可见人物中心。以下偏移来自当前 aizen actions_game 的透明 bbox 审计：
# idle 中心约 +82.5px，开场平均 +88.1px，胜利平均 +80.3px；再乘 ORIGINAL_FIGHTER_SCALE。
const ORIGINAL_CAMERA_FOCUS_ONE_IDLE_CENTER_Y_FROM_NODE: float = 82.5 * ORIGINAL_FIGHTER_SCALE
const ORIGINAL_CAMERA_FOCUS_ONE_INTRO_CENTER_Y_FROM_NODE: float = 88.1 * ORIGINAL_FIGHTER_SCALE
const ORIGINAL_CAMERA_FOCUS_ONE_WIN_CENTER_Y_FROM_NODE: float = 80.3 * ORIGINAL_FIGHTER_SCALE
# mc_023.xml 声音层：开场 label index=1537，语音 sound_005 at 1553；声音 sound_013 at 1540；
# 胜利 label index=1596，语音 sound_009 at 1599。
const ORIGINAL_AIZEN_INTRO_SFX_DELAY: float = (1540.0 - 1537.0) / 30.0
const ORIGINAL_AIZEN_INTRO_VOICE_DELAY: float = (1553.0 - 1537.0) / 30.0
const ORIGINAL_AIZEN_WIN_VOICE_DELAY: float = (1599.0 - 1596.0) / 30.0
const ORIGINAL_AIZEN_INTRO_SFX_PATH: String = "res://assets/sfx/aizen/sound_013_intro_sfx.wav"
const ORIGINAL_AIZEN_INTRO_VOICE_PATH: String = "res://assets/sfx/aizen/sound_005_intro_voice.wav"
const ORIGINAL_AIZEN_WIN_VOICE_PATH: String = "res://assets/sfx/aizen/sound_009_win_voice.wav"
...
# Step64：开场 / 胜利镜头特写状态。
var original_start_sequence_active: bool = false
var original_start_step: int = -1
var original_start_hold_timer: float = 0.0
var original_start_intro_started: bool = false
var original_camera_focus_one_active: bool = false
var original_camera_focus_one_target: Node2D = null
var original_round_end_winner_action_started: bool = false
# Step66：按原版时间轴延迟播放“开场/胜利”声音层，不直接用立即播放替代。
var original_timeline_sfx_events: Array = []
...
# Step82：原版 setAttack()/setSkill2() 是时间轴打开的连招窗口；
# 玩家提前按下的 justDownDelay 输入会在窗口出现的同一帧被消化，而不是等动作播完。
var p1_next_attack_ready: bool = false
var p1_next_attack_action: String = ""
var p1_chain_skill2_ready: bool = false
...
var p2_air_skill_ready: bool = false
var p2_air_bisha_ready: bool = false
...
var p2_active_original_hitbox: String = ""
var p2_active_attacker_follow_target: bool = false
var p2_active_attacker_effect_name: String = ""
var p2_active_attacker_offset: Vector2 = Vector2.ZERO
var p2_active_attacker_original_origin: Vector2 = Vector2.ZERO
var p2_active_attacker_facing: int = -1
var p2_active_attacker_start_ms: int = 0
var p2_pending_hit_target_checker: String = ""
var p2_pending_hit_target_action: String = ""
var p2_pending_hit_target_enabled: bool = false
var p2_attack_skill2_1_ready: bool = false
var p2_attack_skill2_next_attack_ready: bool = false
var p2_next_attack_ready: bool = false
var p2_next_attack_action: String = ""
var p2_chain_skill2_ready: bool = false
var p2_zhao1_ready: bool = false

# Step126：Naruto mc_ctrler.fire(zh1atm/zh2atm) 原版 Bullet 状态。
# fire() 不是普通主时间轴 hitbox；它会把 zh1atm/zh2atm 转成 Bullet，按 start/hit/hold/hits 移动和重复命中。
var p1_original_bullets: Array = []
var p2_original_bullets: Array = []
...
func get_character_manifest_path(char_id: String) -> String:
	var id: String = char_id.to_lower()
	if id == "aizen":
		return "res://assets/characters/aizen/aizen_game_manifest.json"
	if id == "naruto":
		return "res://assets/characters/naruto/naruto_game_manifest.json"
	return ""


func get_character_effect_manifest_path(char_id: String) -> String:
	var id: String = char_id.to_lower()
	if id == "aizen":
		return "res://assets/characters/aizen/aizen_effect_manifest.json"
	if id == "naruto":
		return "res://assets/characters/naruto/naruto_effect_manifest.json"
	return ""


func get_fighter_character_id(fighter: Variant, fallback: String = "aizen") -> String:
	if fighter != null and is_instance_valid(fighter) and fighter is Object:
		var obj := fighter as Object
		if obj.has_meta("character_id"):
			return str(obj.get_meta("character_id"))
		if obj.has_method("get_original_character_id"):
			return str(obj.call("get_original_character_id"))
	return fallback


func create_manifest_fighter_for_character(char_id: String, pos: Vector2, facing_dir: int):
	var manifest_path: String = get_character_manifest_path(char_id)
	if manifest_path == "" or not FileAccess.file_exists(manifest_path):
		print("Manifest not found for character, fallback placeholder: ", char_id, " path=", manifest_path)
		return create_placeholder_player(char_id, pos)

	var fighter = ManifestFighter.new()
	fighter.set_meta("character_id", char_id.to_lower())
	game_layer.add_child(fighter)
	fighter.position = pos
	fighter.ground_y = GROUND_Y
	fighter.scale = Vector2(ORIGINAL_FIGHTER_SCALE, ORIGINAL_FIGHTER_SCALE)
	fighter.z_index = 10
	fighter.setup(manifest_path)
	apply_original_default_battle_stats(fighter)
	fighter.set_facing(facing_dir)

	# Stage89：Naruto 先接入正常形态主动作与基础控制参数。后续再单独补齐专属攻击框/特效。
	if char_id.to_lower() == "naruto":
		fighter.set("jump_speed", -650.0)
		fighter.set_meta("original_speed", 8)
		fighter.set_meta("original_heavy", 3)
		fighter.set_meta("original_defense_type", 1)

	return fighter


func spawn_fighters() -> void:
	p1 = create_manifest_fighter_for_character(GameData.p1_character, P1_START_POS, 1)
	if p1 != null and p1.has_signal("frame_event"):
		p1.frame_event.connect(_on_p1_frame_event)

	p2 = create_manifest_fighter_for_character(GameData.p2_character, P2_START_POS, -1)
	if p2 != null and p2.has_signal("frame_event"):
		p2.frame_event.connect(_on_p2_frame_event)
...
func setup_effect_player_for_character(effect_player: EffectPlayer, char_id: String) -> void:
	var path: String = get_character_effect_manifest_path(char_id)
	if path != "" and FileAccess.file_exists(path):
		effect_player.setup(path)
	else:
		print("Effect manifest not found for ", char_id, ", using Aizen fallback only if present. path=", path)
		if FileAccess.file_exists("res://assets/characters/aizen/aizen_effect_manifest.json"):
			effect_player.setup("res://assets/characters/aizen/aizen_effect_manifest.json")
...
func spawn_effect_players() -> void:
	...
	p1_effect.setup(get_character_effect_manifest_path(get_fighter_character_id(p1, GameData.p1_character)))

	p2_effect = EffectPlayer.new()
	p2_effect.z_index = 20
	game_layer.add_child(p2_effect)
	p2_effect.setup(get_character_effect_manifest_path(get_fighter_character_id(p2, GameData.p2_character)))
...
func _process(delta: float) -> void:
	...
	update_original_timeline_sfx_events(delta)
	...
	if ko_finished or original_round_end_actions_applied:
		...
		if ko_anim_phase == 6 and not round_end_menu_shown:
			show_round_end_menu()
	...
	update_p2_attacker_follow_target()
	...
	update_p2_pending_hit_target_check()
	check_active_hit_target()
	update_p2_test_attack_box()
	update_original_naruto_bullets(delta * get_original_slow_time_scale())
	update_hp_ui()
	...
	check_ko_state()
	# Step71：KO 以后原版切到 GameEndCtrl/FightUI 结算流程，战斗输入、持续攻击面、身体分离都不应再运行。
	# 旧逻辑只让 handle_p1_input/handle_p2_input 因 ko_finished return，但后面的 resolve_original_cross_collision 仍可能在输入锁结束后推开胜者，
	# 造成“胜利动作播完后突然移动一段距离”。这里保留相机/KO 序列更新，直接跳过剩余战斗逻辑。
	if ko_finished or original_round_end_actions_applied:
		# Step78：原版 GameMainLogicCtrler 在 GameEndCtrl 结算期间仍会运行 renderGameSprite()，
		# 其中 GameLogic.fixGameSpritePosition() 会把败者夹回地图/镜头边界。
		# 但不再运行身体分离，避免回合结束后胜者被挤动。
	...
	if fight_input_lock_timer > 0.0:
		update_original_camera(delta)
		return
	if p1 != null and not as_bool(p1.get("in_air")):
		p1_air_skill_ready = false
		p1_air_bisha_ready = false
	handle_p1_input(delta * get_original_slow_time_scale())
	handle_p2_input(delta * get_original_slow_time_scale())
	# 原版每帧先 fixGameSpritePosition，再 checkBodyHit；这里先夹边界并记录 isTouchSide，
	# 再按原版重叠/速度/重量逻辑做双方身体挤压。
	clamp_fighters_to_original_map_bounds()
...
func is_original_aizen_fighter(fighter: Variant) -> bool:
	# 当前 2P Demo 双方都是 Aizen；这里仍按 manifest_path 做保护，避免以后其他角色误播蓝染时间轴语音。
	if fighter == null or not is_instance_valid(fighter):
		return false
	if fighter is Object:
		var manifest_value = (fighter as Object).get("manifest_path")
		if typeof(manifest_value) == TYPE_STRING:
			return String(manifest_value).to_lower().find("aizen") >= 0
	return false


func schedule_original_timeline_sfx(path: String, delay: float, volume_db: float = -2.0, tag: String = "") -> void:
	if path == "":
		return
	...
	original_timeline_sfx_events.append(event)


func clear_original_timeline_sfx_events() -> void:
	original_timeline_sfx_events.clear()


func update_original_timeline_sfx_events(delta: float) -> void:
	for i: int in range(original_timeline_sfx_events.size() - 1, -1, -1):
		var event: Dictionary = original_timeline_sfx_events[i]
		var remain: float = float(event.get("time", 0.0)) - delta
		if remain <= 0.0:
			var path: String = String(event.get("path", ""))
			var volume: float = float(event.get("volume", -2.0))
			if path != "":
				AudioManager.play_sfx(path, volume)
			original_timeline_sfx_events.remove_at(i)
		else:
			event["time"] = remain
			original_timeline_sfx_events[i] = event


func play_original_aizen_intro_timeline_sounds(fighter: Variant) -> void:
	# 原版 mc_023.xml：开场 label index=1537；
	# “声音”层 sound_013 at 1540，“语音”层 sound_005 at 1553。
	if not is_original_aizen_fighter(fighter):
		return
	schedule_original_timeline_sfx(ORIGINAL_AIZEN_INTRO_SFX_PATH, ORIGINAL_AIZEN_INTRO_SFX_DELAY, -3.0, "aizen_intro_sfx_sound_013")
	schedule_original_timeline_sfx(ORIGINAL_AIZEN_INTRO_VOICE_PATH, ORIGINAL_AIZEN_INTRO_VOICE_DELAY, -2.0, "aizen_intro_voice_sound_005")


func play_original_aizen_win_timeline_sounds(fighter: Variant) -> void:
	# 原版 mc_023.xml：胜利 label index=1596；“语音”层 sound_009 at 1599。
	if not is_original_aizen_fighter(fighter):
		return
	schedule_original_timeline_sfx(ORIGINAL_AIZEN_WIN_VOICE_PATH, ORIGINAL_AIZEN_WIN_VOICE_DELAY, -2.0, "aizen_win_voice_sound_009")


func original_camera_focus_one(fighter: Variant, immediate: bool = false) -> void:
	# GameStage.cameraFocusOne(display)：camera.focus([display]); camera.setZoom(3.5); tweenSpd=2.5。
	if fighter != null and is_instance_valid(fighter) and fighter is Node2D:
		original_camera_focus_one_target = fighter as Node2D
		original_camera_focus_one_active = true
		if immediate:
			update_original_camera(0.0, true)


func original_camera_resume() -> void:
	# GameStage.cameraResume()：恢复双人 focus。
	original_camera_focus_one_active = false
	original_camera_focus_one_target = null


func _camera_focus_one_point(node: Variant, fallback: Vector2) -> Vector2:
	# 原版 GameStage.cameraFocusOne(display) 传入的是 FighterMain.getDisplay()，GameCamera.renderY 使用：
	# fy = display.y - screenHalf / zoom - mapBottomDistance。
	# 但当前 Godot 的 ManifestFighter 是 centered PNG：Node2D.position 对应导出帧中心/注册点，
	# 不等价于 Flash 显示对象的视觉中心。若直接聚焦 Node2D.position，会只看到半个人物。
	# 因此这里根据当前动作透明 bbox 的可见人物中心做一次坐标映射，不改变角色世界坐标、hitbox 或镜头公式。
	var pos: Vector2 = get_node_ground_pos(node, fallback)
	var offset: Vector2 = get_original_camera_focus_one_visual_offset(node)
	return pos + offset


func get_original_camera_focus_one_visual_offset(node: Variant) -> Vector2:
	var action_name: String = ""
	if node != null and is_instance_valid(node) and node is Object:
		var action_value = (node as Object).get("current_action")
		if typeof(action_value) == TYPE_STRING:
			action_name = String(action_value)
	match action_name:
		"intro":
			return Vector2(0.0, ORIGINAL_CAMERA_FOCUS_ONE_INTRO_CENTER_Y_FROM_NODE)
		"win":
			return Vector2(0.0, ORIGINAL_CAMERA_FOCUS_ONE_WIN_CENTER_Y_FROM_NODE)
		_:
			return Vector2(0.0, ORIGINAL_CAMERA_FOCUS_ONE_IDLE_CENTER_Y_FROM_NODE)


func update_original_camera_focus_one(_delta: float, immediate: bool = false) -> void:
	if game_layer == null:
		return
	if original_camera_focus_one_target == null or not is_instance_valid(original_camera_focus_one_target):
		original_camera_resume()
		update_original_camera(_delta, immediate)
		return
	var fallback := P1_START_POS
	if original_camera_focus_one_target == p2:
		fallback = P2_START_POS
	var focus_center := _camera_focus_one_point(original_camera_focus_one_target, fallback)
	var target_zoom: float = maxf(0.001, ORIGINAL_CAMERA_FOCUS_ONE_ZOOM * GODOT_XIANSHI_CAMERA_VISUAL_SCALE)
	...
	camera_target_zoom = target_zoom
	camera_target_logic_zoom = ORIGINAL_CAMERA_FOCUS_ONE_ZOOM
	camera_target_rect = target_rect
...
func update_original_camera(_delta: float, immediate: bool = false) -> void:
	if original_camera_focus_one_active:
		update_original_camera_focus_one(_delta, immediate)
		return
	...
	camera_target_zoom = target_zoom
	camera_target_logic_zoom = original_logic_zoom
	camera_target_rect = target_rect
...
func start_round_intro() -> void:
	# 原版 GameStartCtrler.start1v1：Round/Fight UI 之前先分别 cameraFocusOne 并播放双方“开场”动作。
	game_time_left = float(DEMO_GAME_TIME_MAX)
	time_over_finished = false
	ko_score_added = false
	round_intro_timer = 0.0
	round_intro_phase = -1
	fight_input_lock_timer = 999.0
	original_start_sequence_active = true
	original_start_step = 0
	original_start_hold_timer = 0.0
	original_start_intro_started = false
	original_round_end_winner_action_started = false
	clear_original_timeline_sfx_events()
	original_camera_resume()
	if fight_hud != null:
	...
	print("原版 GameStartCtrler：cameraFocusOne(P1)->P1.sayIntro()->cameraFocusOne(P2)->P2.sayIntro()->cameraResume()->FightUI.showStart。")


func begin_round_start_ui() -> void:
	# 原版 FightUI.showStart：startKOmc.gotoAndStop("start")；frame54 fight_in，frame57 fight，frame87 complete。
...
func update_original_start_sequence(delta: float) -> void:
	if not original_start_sequence_active:
		return
	fight_input_lock_timer = 999.0
	if original_start_hold_timer > 0.0:
		original_start_hold_timer = maxf(0.0, original_start_hold_timer - delta)
		return

	match original_start_step:
		0:
			# GameStartCtrler.renderStart1v1 case 0：cameraFocusOne(P1)，hold 0.5s。
			original_camera_focus_one(p1, true)
			original_start_hold_timer = ORIGINAL_START_FOCUS_HOLD_TIME
			original_start_step = 1
		1:
			# case 1：P1.sayIntro() -> FighterMcCtrler.sayIntro() -> goFrame("开场")。
			if not original_start_intro_started:
				if p1 != null and p1.has_method("play_action"):
					node_play_action_safe(p1, "intro")
					play_original_aizen_intro_timeline_sounds(p1)
				original_start_intro_started = true
				return
			if p1 == null or str(p1.get("current_action")) != "intro":
				original_start_intro_started = false
				original_start_hold_timer = ORIGINAL_START_AFTER_INTRO_HOLD_TIME
				original_start_step = 2
		2:
			# case 2：cameraFocusOne(P2)，hold 0.5s。
			original_camera_focus_one(p2, true)
			original_start_hold_timer = ORIGINAL_START_FOCUS_HOLD_TIME
			original_start_step = 3
		3:
			# case 3：P2.sayIntro()。
			if not original_start_intro_started:
				if p2 != null and p2.has_method("play_action"):
					node_play_action_safe(p2, "intro")
					play_original_aizen_intro_timeline_sounds(p2)
				original_start_intro_started = true
				return
			if p2 == null or str(p2.get("current_action")) != "intro":
				original_start_intro_started = false
				original_start_hold_timer = ORIGINAL_START_AFTER_INTRO_HOLD_TIME
				original_start_step = 4
		4:
			# case 4：cameraResume()，hold 0.1s。
			original_camera_resume()
			original_start_hold_timer = ORIGINAL_START_RESUME_HOLD_TIME
			original_start_step = 5
		5:
			# case 5：FightUI.showStart，即 Round/Fight UI。
			original_start_sequence_active = false
			original_start_step = -1
			begin_round_start_ui()


func update_round_intro(delta: float) -> void:
	if original_start_sequence_active:
		update_original_start_sequence(delta)
		return
	if fight_input_lock_timer > 0.0:
...
func get_original_winner_action_hold_time(winner: Variant) -> float:
	if winner == null:
		return ORIGINAL_WIN_ACTION_HOLD_TIME
	var winner_char_id: String = get_fighter_character_id(winner, "")
	if winner_char_id == "aizen":
		return ORIGINAL_WIN_ACTION_HOLD_TIME
	if winner.has_method("get_action_duration_seconds"):
		var duration: float = float(winner.call("get_action_duration_seconds", "win"))
		if duration > 0.0:
			# 多给 0.05 秒，避免最后一帧与结算菜单同帧抢显示。
...
func update_ko_sequence(delta: float) -> void:
	...
	# Step64：保留自建结算菜单，但按原版 GameEndCtrler 在菜单前增加败者倒地、胜者镜头聚焦和胜利动作。
	force_clear_bisha_after_round_end()
	force_hide_legacy_blackout_layers()
	...
	if ko_anim_phase == 1:
		# FightUI.playKO：500ms 预延迟后播放 ko / timeover 动画与声音。
		# 原版 startKOmc 会进入 ko label：语音播报期间画面上也应出现 KO 字样。
		ko_anim_phase = 2
		ko_sequence_timer = TIMEOVER_ANIM_TIME if ko_timeover else KO_ANIM_TIME
	...
	if ko_anim_phase == 2:
		# FightUI.renderEnd：KO 后等待败者进入 DEAD/倒地，再延迟 1~2 秒展示 Winner。
		ko_anim_phase = 3
	...
	if ko_anim_phase == 3:
		if ko_draw_game or ko_winner_id == 0:
			...
			show_round_end_menu()
			return
		# 原版 FightUI.renderEnd：先等 loser.actionState 进入 DEAD，再延迟展示 winner。
		# 如果败者还在被击飞/弹起，就不要过早切胜者特写。
		if not is_original_loser_dead_ready():
			ko_sequence_timer = 0.05
			return
		# FightUI.renderEnd 到达 _showWinnerDelay 后：cameraFocusOne(winner)，showWinner(winner)。
		var winner = get_original_round_winner()
		if winner != null:
			original_camera_focus_one(winner, false)
		ko_anim_phase = 4
		ko_sequence_timer = ORIGINAL_WINNER_FOCUS_HOLD_TIME
		hide_fight_hud_center_message()
		return

	if ko_anim_phase == 4:
		# GameEndCtrler.renderEND step1：确认可胜利动作后调用 _winner.win()，播放“胜利”动作。
		# Step131：Aizen 仍用 win/0033 的用户确认截停时间；Naruto 按 mc_0046 + mc_0047 展开帧数等待。
		var winner2 = get_original_round_winner()
		if winner2 != null and winner2.has_method("play_action"):
			winner2.play_action("win")
			play_original_aizen_win_timeline_sounds(winner2)
		original_round_end_winner_action_started = true
		ko_anim_phase = 5
		ko_sequence_timer = get_original_winner_action_hold_time(winner2)
		return
...
func refresh_original_camera_target_state() -> void:
	# 对齐原版 GameCamera：fixGameSpritePosition() 使用 getZoom()/getScreenRect() 默认值，
	# 也就是 _zoom / _noTweenRect（目标镜头），不是带 tween 的显示镜头。
	if original_camera_focus_one_active:
		if original_camera_focus_one_target == null or not is_instance_valid(original_camera_focus_one_target):
			return
		var focus_fallback := P1_START_POS
		if original_camera_focus_one_target == p2:
			focus_fallback = P2_START_POS
		var focus_center_one := _camera_focus_one_point(original_camera_focus_one_target, focus_fallback)
		var focus_target_zoom: float = maxf(0.001, ORIGINAL_CAMERA_FOCUS_ONE_ZOOM * GODOT_XIANSHI_CAMERA_VISUAL_SCALE)
		var focus_view_size := ORIGINAL_GAME_SIZE / focus_target_zoom
		var focus_target_rect := Rect2(Vector2.ZERO, focus_view_size)
		focus_target_rect.position.x = focus_center_one.x - focus_view_size.x * 0.5
		focus_target_rect.position.y = focus_center_one.y - focus_view_size.y * 0.5 - ORIGINAL_CAMERA_OFFSET_Y
		var focus_min_x := ORIGINAL_MAP_LEFT
		var focus_max_x := maxf(focus_min_x, ORIGINAL_MAP_RIGHT - focus_view_size.x)
		var focus_min_y := -1000.0 * focus_target_zoom
		var focus_max_y := maxf(focus_min_y, ORIGINAL_MAP_BOTTOM - focus_view_size.y)
		focus_target_rect.position.x = clampf(focus_target_rect.position.x, focus_min_x, focus_max_x)
		focus_target_rect.position.y = clampf(focus_target_rect.position.y, focus_min_y, focus_max_y)
		camera_target_zoom = focus_target_zoom
		camera_target_logic_zoom = ORIGINAL_CAMERA_FOCUS_ONE_ZOOM
		camera_target_rect = focus_target_rect
		return
...
func clamp_fighters_to_original_map_bounds() -> void:
	refresh_original_camera_target_state()
	clamp_fighter_to_original_map_bounds(p1)
	clamp_fighter_to_original_map_bounds(p2)


func get_original_node_vec_x(node: Variant) -> float:
	if node == null:
		return 0.0
	if not is_instance_valid(node):
		return 0.0
	var value: Variant = node.get("velocity_x")
	if typeof(value) == TYPE_INT or typeof(value) == TYPE_FLOAT:
		var vx := float(value)
		if absf(vx) > 0.001:
			return vx
	# 当前 Godot 地面 walk 是 BattlePlaceholder 直接改 position，不会写入 ManifestFighter.velocity_x；
	# 但原版 FighterMain.getVecX() 在走路时非 0。这里为边界判断/身体挤压补回 walk 速度。
	if node is Object:
		var action_value: Variant = (node as Object).get("current_action")
		if typeof(action_value) == TYPE_STRING and String(action_value) == "walk":
			var facing_value: Variant = (node as Object).get("facing")
			var facing_dir: float = 1.0
			if typeof(facing_value) == TYPE_INT or typeof(facing_value) == TYPE_FLOAT:
				facing_dir = -1.0 if float(facing_value) < 0.0 else 1.0
			return move_speed * facing_dir
	return 0.0


func is_original_node_in_air(node: Variant) -> bool:
	if node == null:
		return false
	if not is_instance_valid(node):
		return false
	var value: Variant = node.get("in_air")
	if typeof(value) == TYPE_BOOL:
		return as_bool(value)
	if node is Node2D:
		return (node as Node2D).position.y < GROUND_Y - 0.5
	return false


func set_original_touch_side(node: Variant, touched: bool) -> void:
	if node == null or not is_instance_valid(node):
		return
	if node is Object:
		(node as Object).set_meta("original_is_touch_side", touched)


func get_original_touch_side(node: Variant) -> bool:
	if node == null or not is_instance_valid(node):
		return false
	if node is Object and (node as Object).has_meta("original_is_touch_side"):
		return as_bool((node as Object).get_meta("original_is_touch_side"))
	return false


func clamp_fighter_to_original_map_bounds(fighter: Variant) -> void:
	# 对照原版 GameLogic.fixGameSpritePosition(sp)：
	# left = map.left + GameConfig.X_SIDE_OFFSET; right = map.right - GameConfig.X_SIDE_OFFSET。
	# 如果 FighterMain.getVecX()!=0，且 GameCamera 目标 zoom 已到 autoZoomMin，
	# 使用 GameCamera.getScreenRect() 的默认值 _noTweenRect 再限制一层可见区域。
	if fighter == null or not is_instance_valid(fighter):
		return
	...
	var left_limit: float = ORIGINAL_MAP_LEFT + ORIGINAL_MAP_X_SIDE_OFFSET
	var right_limit: float = ORIGINAL_MAP_RIGHT - ORIGINAL_MAP_X_SIDE_OFFSET
	var offset_x: float = ORIGINAL_MAP_X_SIDE_OFFSET

	var original_logic_zoom: float = camera_target_logic_zoom
	var target_rect: Rect2 = camera_target_rect
	if target_rect.size.x <= 0.0:
		target_rect = camera_rect
	var is_moving_or_air: bool = absf(get_original_node_vec_x(node)) > 0.001 or is_original_node_in_air(node)
	if is_moving_or_air and original_logic_zoom <= ORIGINAL_CAMERA_AUTO_ZOOM_MIN + 0.001:
		# 原版公式：camLeft = camRect.x / camzoom + offsetX; camRight = camLeft + camRect.width - offsetX。
		# 在 autoZoomMin=1 时等价于当前 800px 目标可见区域。
		var cam_left: float = target_rect.position.x / maxf(0.001, original_logic_zoom) + offset_x
		var cam_right: float = cam_left + target_rect.size.x - offset_x
		if left_limit < cam_left:
			left_limit = cam_left
		if right_limit > cam_right:
			right_limit = cam_right

	var is_touch_side: bool = false
	if node.position.x <= left_limit:
		node.position.x = left_limit
		is_touch_side = true
		var vx_left: Variant = node.get("velocity_x")
		if (typeof(vx_left) == TYPE_INT or typeof(vx_left) == TYPE_FLOAT) and float(vx_left) < 0.0:
			node.set("velocity_x", 0.0)
			node.set("damping_x", 0.0)
	if node.position.x >= right_limit:
		node.position.x = right_limit
		is_touch_side = true
		var vx_right: Variant = node.get("velocity_x")
		if (typeof(vx_right) == TYPE_INT or typeof(vx_right) == TYPE_FLOAT) and float(vx_right) > 0.0:
			node.set("velocity_x", 0.0)
			node.set("damping_x", 0.0)
	set_original_touch_side(node, is_touch_side)
...
func handle_p1_input(delta: float) -> void:
	...
	if get_node_current_action_safe(p1) == "defend" and not pure_down_defense:
		node_play_action_safe(p1, "defend_recover")
	elif pure_down_defense and node_can_ground_action_safe(p1):
		node_play_action_safe(p1, "defend")
		return

	# 原版 renderFloorAction：只有按住朝向目标的移动方向，并且 catch1/catch2 输入成立时，才允许 doCatch。
	# Step22-24：改为吃输入缓冲，提前几帧按下也能在可行动第一帧立即触发。
	if p1 != null and node_can_ground_action_safe(p1):
	...
	# 原版 renderAirAction 末尾允许空中水平移动；不切 walk 动画。
	if get_node_in_air_safe(p1):
		handle_p1_air_move(delta)
		return

	# A / D：只在地面空闲时切 walk；攻击/技能/必杀动作中不覆盖当前动画。
	if node_can_ground_action_safe(p1):
		if Input.is_key_pressed(KEY_A):
			p1.position.x -= move_speed * delta
			clamp_fighter_to_original_map_bounds(p1)
			node_set_facing_safe(p1, -1)
			if get_node_current_action_safe(p1) != "walk":
				node_play_action_safe(p1, "walk")
			...
			if get_node_current_action_safe(p1) != "walk":
				node_play_action_safe(p1, "walk")
		elif get_node_current_action_safe(p1) != "idle":
			node_play_action_safe(p1, "idle")


func p2_key_up_pressed() -> bool:
	# 原版 ConfigVO.as: P2 up = Keyboard.UP。
	# Step70：方向键不能只读 physical_key；部分系统/Godot 对非字符键的 physical 状态不稳定。
	# 同时读 logical + physical，但不把小键盘 8 的 NumLock 关闭状态映射成 up，避免再次误触发。
	return Input.is_key_pressed(KEY_UP) or Input.is_physical_key_pressed(KEY_UP)


func p2_key_down_pressed() -> bool:
	# 原版 ConfigVO.as: P2 down = Keyboard.DOWN。
	return Input.is_key_pressed(KEY_DOWN) or Input.is_physical_key_pressed(KEY_DOWN)


func p2_key_left_pressed() -> bool:
	# 原版 ConfigVO.as: P2 left = Keyboard.LEFT。
	return Input.is_key_pressed(KEY_LEFT) or Input.is_physical_key_pressed(KEY_LEFT)


func p2_key_right_pressed() -> bool:
	# 原版 ConfigVO.as: P2 right = Keyboard.RIGHT。
	return Input.is_key_pressed(KEY_RIGHT) or Input.is_physical_key_pressed(KEY_RIGHT)


func p2_key_attack_pressed() -> bool:
	# 原版 ConfigVO.as: P2 attack = Keyboard.NUMPAD_1。
	# 当前项目额外允许主键盘 1 作为无独立小键盘设备的兜底。
	return Input.is_key_pressed(KEY_KP_1) or Input.is_physical_key_pressed(KEY_KP_1) or Input.is_key_pressed(KEY_1) or Input.is_physical_key_pressed(KEY_1)


func p2_key_jump_pressed() -> bool:
	# 原版 ConfigVO.as: P2 jump = Keyboard.NUMPAD_2。
	return Input.is_key_pressed(KEY_KP_2) or Input.is_physical_key_pressed(KEY_KP_2) or Input.is_key_pressed(KEY_2) or Input.is_physical_key_pressed(KEY_2)


func p2_key_dash_pressed() -> bool:
	# 原版 ConfigVO.as: P2 dash = Keyboard.NUMPAD_3。
	return Input.is_key_pressed(KEY_KP_3) or Input.is_physical_key_pressed(KEY_KP_3) or Input.is_key_pressed(KEY_3) or Input.is_physical_key_pressed(KEY_3)


func p2_key_skill_pressed() -> bool:
	# 原版 ConfigVO.as: P2 skill = Keyboard.NUMPAD_4。
	return Input.is_key_pressed(KEY_KP_4) or Input.is_physical_key_pressed(KEY_KP_4) or Input.is_key_pressed(KEY_4) or Input.is_physical_key_pressed(KEY_4)


func p2_key_bisha_pressed() -> bool:
	# 原版 ConfigVO.as: P2 superKill = Keyboard.NUMPAD_5。
	return Input.is_key_pressed(KEY_KP_5) or Input.is_physical_key_pressed(KEY_KP_5) or Input.is_key_pressed(KEY_5) or Input.is_physical_key_pressed(KEY_5)


func handle_p2_input(delta: float) -> void:
	# 原版 ConfigVO.as P2 默认键位：↑/↓/←/→ + Num1/Num2/Num3/Num4/Num5/Num6。
	# Num6/O 是 beckons/assist，当前仍不接支援/换人系统。
	if ko_finished:
		return
	if p2 == null or not p2.has_method("play_action"):
		return

	var attack_pressed: bool = p2_key_attack_pressed()
	var jump_pressed: bool = p2_key_jump_pressed()
	var dash_pressed: bool = p2_key_dash_pressed()
	var skill_pressed: bool = p2_key_skill_pressed()
	var bisha_pressed: bool = p2_key_bisha_pressed()

	var attack_just: bool = attack_pressed and not prev_p2_attack_pressed
	var jump_just: bool = jump_pressed and not prev_p2_jump_pressed
	var dash_just: bool = dash_pressed and not prev_p2_dash_pressed
	var skill_just: bool = skill_pressed and not prev_p2_skill_pressed
	var bisha_just: bool = bisha_pressed and not prev_p2_bisha_pressed

	prev_p2_attack_pressed = attack_pressed
	prev_p2_jump_pressed = jump_pressed
	prev_p2_dash_pressed = dash_pressed
	prev_p2_skill_pressed = skill_pressed
	prev_p2_bisha_pressed = bisha_pressed

	update_p2_input_buffers(delta, attack_just, jump_just, dash_just, skill_just, bisha_just)

	# 原版 renderFloorAction 优先级：bishaSUPER -> bishaUP -> bisha -> skill -> attack -> defense。
	if p2_bisha_buffer_timer > 0.0 and try_execute_one_p2_buffer("bisha"):
		p2_bisha_buffer_timer = 0.0
		return

	var pure_down_defense: bool = p2_key_down_pressed() and not (attack_pressed or skill_pressed or bisha_pressed or jump_pressed or dash_pressed)
	if get_node_current_action_safe(p2) == "defend" and not pure_down_defense:
		node_play_action_safe(p2, "defend_recover")
	elif pure_down_defense and node_can_ground_action_safe(p2):
		node_play_action_safe(p2, "defend")
		return

	try_execute_p2_buffered_actions()

	# 原版 renderAirAction 末尾允许空中水平移动；不切 walk 动画。
	if get_node_in_air_safe(p2):
		handle_p2_air_move(delta)
		return

	if node_can_ground_action_safe(p2):
		if p2_key_left_pressed():
			p2.position.x -= move_speed * delta
			clamp_fighter_to_original_map_bounds(p2)
			node_set_facing_safe(p2, -1)
			if get_node_current_action_safe(p2) != "walk":
				node_play_action_safe(p2, "walk")
		elif p2_key_right_pressed():
			p2.position.x += move_speed * delta
			clamp_fighter_to_original_map_bounds(p2)
			node_set_facing_safe(p2, 1)
			if get_node_current_action_safe(p2) != "walk":
				node_play_action_safe(p2, "walk")
		elif get_node_current_action_safe(p2) != "idle":
			node_play_action_safe(p2, "idle")


func clear_p1_original_attack_chain_window() -> void:
	p1_next_attack_ready = false
	p1_next_attack_action = ""
	p1_chain_skill2_ready = false


func clear_p2_original_attack_chain_window() -> void:
	p2_next_attack_ready = false
	p2_next_attack_action = ""
	p2_chain_skill2_ready = false


func try_consume_p1_original_attack_chain(up_pressed: bool, down_pressed: bool) -> bool:
	if p1 == null:
		return false
	# 原版 setSkill2() 默认打开 W+J / 砍技2。该窗口在 atk2 时间轴第 8 帧出现，
	# 不能等待 can_ground_action()，否则就会体感“按了很久才出”。
	if up_pressed and p1_chain_skill2_ready:
		clear_p1_original_attack_chain_window()
		attack_skill2_1_ready = false
		attack_skill2_next_attack_ready = false
		p1_zhao1_ready = false
		node_play_action_safe(p1, "attack_skill2")
		return true

	# 原版 setAttack(label) 打开下一段 J；J 不能和 W/S 同时走普通连段。
	if not up_pressed and not down_pressed and p1_next_attack_ready and p1_next_attack_action != "":
		var next_action: String = p1_next_attack_action
		clear_p1_original_attack_chain_window()
		if next_action == "atk4_extra":
			attack_skill2_next_attack_ready = false
		node_play_action_safe(p1, next_action)
		return true

	return false


func try_consume_p2_original_attack_chain(up_pressed: bool, down_pressed: bool) -> bool:
	if p2 == null:
		return false
	if up_pressed and p2_chain_skill2_ready:
		clear_p2_original_attack_chain_window()
		p2_attack_skill2_1_ready = false
		p2_attack_skill2_next_attack_ready = false
		p2_zhao1_ready = false
		node_play_action_safe(p2, "attack_skill2")
		return true

	if not up_pressed and not down_pressed and p2_next_attack_ready and p2_next_attack_action != "":
		var next_action: String = p2_next_attack_action
		clear_p2_original_attack_chain_window()
		if next_action == "atk4_extra":
			p2_attack_skill2_next_attack_ready = false
		node_play_action_safe(p2, next_action)
		return true

	return false


func try_execute_p1_original_chain_window_now() -> void:
	# 时间轴事件和输入处理不一定同一顺序；这里模拟原版同帧 setAttack 后 renderFloorAction 立即消费 justDownDelay。
	if p1_attack_buffer_timer <= 0.0:
		return
	var up_pressed: bool = Input.is_key_pressed(KEY_W)
	var down_pressed: bool = Input.is_key_pressed(KEY_S)
	if try_consume_p1_original_attack_chain(up_pressed, down_pressed):
		p1_attack_buffer_timer = 0.0


func try_execute_p2_original_chain_window_now() -> void:
	if p2_attack_buffer_timer <= 0.0:
		return
	var up_pressed: bool = p2_key_up_pressed()
	var down_pressed: bool = p2_key_down_pressed()
	if try_consume_p2_original_attack_chain(up_pressed, down_pressed):
		p2_attack_buffer_timer = 0.0


func update_p2_input_buffers(delta: float, attack_just: bool, jump_just: bool, dash_just: bool, skill_just: bool, bisha_just: bool) -> void:
	p2_attack_buffer_timer = maxf(0.0, p2_attack_buffer_timer - delta)
	p2_skill_buffer_timer = maxf(0.0, p2_skill_buffer_timer - delta)
	p2_bisha_buffer_timer = maxf(0.0, p2_bisha_buffer_timer - delta)
	p2_jump_buffer_timer = maxf(0.0, p2_jump_buffer_timer - delta)
	p2_dash_buffer_timer = maxf(0.0, p2_dash_buffer_timer - delta)

	if attack_just:
		p2_attack_buffer_timer = ORIGINAL_INPUT_BUFFER_TIME
	if skill_just:
		p2_skill_buffer_timer = ORIGINAL_INPUT_BUFFER_TIME
	if bisha_just:
		p2_bisha_buffer_timer = ORIGINAL_INPUT_BUFFER_TIME
	if jump_just:
		p2_jump_buffer_timer = ORIGINAL_INPUT_BUFFER_TIME
	if dash_just:
		p2_dash_buffer_timer = ORIGINAL_INPUT_BUFFER_TIME


func _did_p2_action_change(before_action: String, before_frame: int) -> bool:
	if p2 == null:
		return false
	return str(get_node_current_action_safe(p2)) != before_action or int(get_node_frame_index_safe(p2)) != before_frame


func try_execute_one_p2_buffer(buffer_name: String) -> bool:
	if p2 == null:
		return false
	var before_action: String = str(get_node_current_action_safe(p2))
	var before_frame: int = int(get_node_frame_index_safe(p2))

	match buffer_name:
		"jump":
			handle_p2_jump_input()
		"dash":
			handle_p2_dash_input()
		"attack":
			handle_p2_attack_input()
		"skill":
			handle_p2_skill_input()
		"bisha":
			handle_p2_bisha_input()
		_:
			return false

	return _did_p2_action_change(before_action, before_frame)


func try_execute_p2_buffered_actions() -> void:
	# 对齐原版 FighterMcCtrler.renderFloorAction 的输入优先级：
	# bishaSUPER / bishaUP / bisha -> skill1/skill2 -> zhao -> attack -> defense -> dash -> move -> jump。
	# 当前 Godot 的 attack 分支同时处理 S+Num1 / ↑+Num1，所以放在 skill 前。
	if p2_bisha_buffer_timer > 0.0 and try_execute_one_p2_buffer("bisha"):
		p2_bisha_buffer_timer = 0.0
		return
	if p2_attack_buffer_timer > 0.0 and try_execute_one_p2_buffer("attack"):
		p2_attack_buffer_timer = 0.0
		return
	if p2_skill_buffer_timer > 0.0 and try_execute_one_p2_buffer("skill"):
		p2_skill_buffer_timer = 0.0
		return
	if p2_dash_buffer_timer > 0.0 and try_execute_one_p2_buffer("dash"):
		p2_dash_buffer_timer = 0.0
		return
	if p2_jump_buffer_timer > 0.0 and try_execute_one_p2_buffer("jump"):
		p2_jump_buffer_timer = 0.0
		return


func sync_p2_input_edge_state() -> void:
	# 清空缓冲时同步 just-pressed 基线，避免暂停/KO/技能锁结束后 P2 因上一帧状态残留出现“按键失效”。
	prev_p2_attack_pressed = p2_key_attack_pressed()
	prev_p2_jump_pressed = p2_key_jump_pressed()
	prev_p2_dash_pressed = p2_key_dash_pressed()
	prev_p2_skill_pressed = p2_key_skill_pressed()
	prev_p2_bisha_pressed = p2_key_bisha_pressed()


func clear_p2_input_buffers() -> void:
	p2_attack_buffer_timer = 0.0
	p2_skill_buffer_timer = 0.0
	p2_bisha_buffer_timer = 0.0
	p2_jump_buffer_timer = 0.0
	p2_dash_buffer_timer = 0.0
	clear_p2_original_attack_chain_window()
	sync_p2_input_edge_state()


func handle_p2_jump_input() -> void:
	if p2 != null and p2.has_method("start_jump"):
		p2.start_jump()


func handle_p2_dash_input() -> void:
	if p2 == null:
		return
	# 原版 renderAirAction 末尾允许空中 dash：doDashAir(瞬步)。
	# doDashAir 条件：_action.jumpTimes >= 1，消耗 energy=30，并把 jumpTimes 清零。
	if get_node_in_air_safe(p2):
		if not p2.has_method("can_original_air_dash") or not as_bool(p2.call("can_original_air_dash")):
			return
		if not try_use_p2_energy(30.0, true, "P2 空中瞬步 / air dash"):
			return
		if p2.has_method("try_start_original_air_dash"):
			p2.call("try_start_original_air_dash")
		return

	if not node_can_ground_action_safe(p2):
		return
	if not try_use_p2_energy(20.0, true, "P2 地面瞬步 / dash"):
		return
	node_play_action_safe(p2, "dash")


func handle_p2_attack_input() -> void:
	var up_pressed: bool = p2_key_up_pressed()
	var down_pressed: bool = p2_key_down_pressed()

	if get_node_in_air_safe(p2):
		p2_air_skill_ready = false
		p2_air_bisha_ready = false
		p2.start_air_attack()
		return
	...
	if get_node_current_action_safe(p2) == "attack_skill2_check" and get_node_frame_index_safe(p2) >= 11 and get_node_frame_index_safe(p2) < 18:
		if up_pressed and p2_attack_skill2_1_ready:
			node_play_action_safe(p2, "attack_skill2_1")
			p2_attack_skill2_1_ready = false
			p2_attack_skill2_next_attack_ready = false
			p2_zhao1_ready = false
			return
		if not up_pressed and not down_pressed and p2_attack_skill2_next_attack_ready:
			play_action_if_available(p2, "atk4_extra", "atk4")
			p2_attack_skill2_next_attack_ready = false
			return

	if down_pressed and node_can_ground_action_safe(p2):
		node_play_action_safe(p2, "attack_skill1")
		return
	if up_pressed and node_can_ground_action_safe(p2):
		p2_attack_skill2_1_ready = false
		p2_attack_skill2_next_attack_ready = false
		clear_p2_original_attack_chain_window()
		node_play_action_safe(p2, "attack_skill2")
	...
	if get_node_current_action_safe(p2) == "atk1" and get_node_frame_index_safe(p2) >= 7 and get_node_frame_index_safe(p2) < 18:
		node_play_action_safe(p2, "atk2")
		return
	if get_node_current_action_safe(p2) == "atk2" and get_node_frame_index_safe(p2) >= 8 and get_node_frame_index_safe(p2) < 16:
		node_play_action_safe(p2, "atk3")
		return
	if get_node_current_action_safe(p2) == "atk3" and get_node_frame_index_safe(p2) >= 11 and get_node_frame_index_safe(p2) < 26:
		node_play_action_safe(p2, "atk4")
		return
	if get_node_current_action_safe(p2) == "atk4_extra" and get_node_frame_index_safe(p2) >= 8 and get_node_frame_index_safe(p2) < 19:
		play_action_if_available(p2, "atk5", "atk4")
		return
	if node_can_ground_action_safe(p2):
		clear_p2_original_attack_chain_window()
		node_play_action_safe(p2, "atk1")


func can_start_p2_ground_bisha() -> bool:
	if p2 == null:
		return false
	if as_bool(p2.get("in_air")):
		return false
	if node_can_ground_action_safe(p2):
		return true
...
func handle_p2_skill_input() -> void:
	var up_pressed: bool = p2_key_up_pressed()
	var down_pressed: bool = p2_key_down_pressed()

	if get_node_in_air_safe(p2):
		# 原版 skillAIR 不等待 jump/fall 动作结束；airHitTimes 可用即可。
		if p2.has_method("can_original_air_action") and not as_bool(p2.call("can_original_air_action")):
			return
		p2_air_skill_ready = false
		p2_air_bisha_ready = false
		if p2.has_method("consume_original_air_action_limits"):
			p2.consume_original_air_action_limits()
		node_play_action_safe(p2, "air_skill")
		return

	if p2_zhao1_ready:
		p2_zhao1_ready = false
		node_play_action_safe(p2, "skill1")
		return

	if not node_can_ground_action_safe(p2):
		return
	if down_pressed:
		node_play_action_safe(p2, "skill2")
	elif up_pressed:
		node_play_action_safe(p2, "skill3")
	else:
		node_play_action_safe(p2, "skill1")


func handle_p2_bisha_input() -> void:
	var up_pressed: bool = p2_key_up_pressed()
	var down_pressed: bool = p2_key_down_pressed()

	if get_node_in_air_safe(p2):
		if fighter_has_action(p2, "super_air"):
			if p2.has_method("can_original_air_action") and not as_bool(p2.call("can_original_air_action")):
				return
			if not try_use_p2_qi(100.0, "P2 空中必杀 / super_air"):
				return
			p2_air_skill_ready = false
			p2_air_bisha_ready = false
			if p2.has_method("consume_original_air_action_limits"):
				p2.consume_original_air_action_limits()
			node_play_action_safe(p2, "super_air")
		return

	if not can_start_p2_ground_bisha():
		return

	if down_pressed:
		if try_use_p2_qi(300.0, "P2 超必杀 / super_special"):
			node_play_action_safe(p2, "super_special")
	elif up_pressed:
		if try_use_p2_qi(100.0, "P2 上必杀 / super_up"):
			node_play_action_safe(p2, "super_up")
	else:
		if try_use_p2_qi(100.0, "P2 普通必杀 / super"):
			node_play_action_safe(p2, "super")
...
func _did_p1_action_change(before_action: String, before_frame: int) -> bool:
	if p1 == null:
		return false
	return str(get_node_current_action_safe(p1)) != before_action or int(get_node_frame_index_safe(p1)) != before_frame


func try_execute_one_p1_buffer(buffer_name: String) -> bool:
	if p1 == null:
		return false
	var before_action: String = str(get_node_current_action_safe(p1))
	var before_frame: int = int(get_node_frame_index_safe(p1))
...
func try_execute_p1_buffered_actions() -> void:
	# 对齐原版 FighterMcCtrler.renderFloorAction 的输入优先级：
	# bishaSUPER / bishaUP / bisha -> skill1/skill2 -> zhao -> attack -> defense -> dash -> move -> jump。
	# 当前 Godot 的 attack 分支同时处理 S+J / W+J，所以放在 skill 前。
	if p1_bisha_buffer_timer > 0.0 and try_execute_one_p1_buffer("bisha"):
		p1_bisha_buffer_timer = 0.0
		return
...
func handle_p2_debug_input() -> void:
	...
	if p2.has_method("can_ground_action") and not as_bool(p2.call("can_ground_action")):
...
func handle_dash_input() -> void:
	...
	# doDashAir 条件：_action.jumpTimes >= 1，消耗 energy=30，并把 jumpTimes 清零。
	if get_node_in_air_safe(p1):
		if not p1.has_method("can_original_air_dash") or not as_bool(p1.call("can_original_air_dash")):
			return
		if not try_use_p1_energy(30.0, true, "空中瞬步 / air dash"):
			return
		if p1.has_method("try_start_original_air_dash"):
			p1.call("try_start_original_air_dash")
		return

	if not node_can_ground_action_safe(p1):
		return
	...
	node_play_action_safe(p1, "dash")
	if ORIGINAL_DASH_INPUT_MANUAL_DISTANCE != 0.0:
		p1.position.x += ORIGINAL_DASH_INPUT_MANUAL_DISTANCE * get_node_facing(p1)
		clamp_fighter_to_original_map_bounds(p1)


func fighter_has_action(fighter: Variant, action_name: String) -> bool:
	return fighter != null and is_instance_valid(fighter) and fighter.has_method("has_original_action") and as_bool(fighter.call("has_original_action", action_name))


func play_action_if_available(fighter: Variant, action_name: String, fallback_action: String = "") -> bool:
	if fighter == null or not is_instance_valid(fighter) or not fighter.has_method("play_action"):
		return false
	if fighter_has_action(fighter, action_name):
		fighter.play_action(action_name)
		return true
	if fallback_action != "" and fighter_has_action(fighter, fallback_action):
		fighter.play_action(fallback_action)
		return true
	return false
...
func handle_attack_input() -> void:
	...
	if get_node_current_action_safe(p1) == "attack_skill2_check" and get_node_frame_index_safe(p1) >= 11 and get_node_frame_index_safe(p1) < 18:
		if up_pressed and attack_skill2_1_ready:
			node_play_action_safe(p1, "attack_skill2_1")
			attack_skill2_1_ready = false
			attack_skill2_next_attack_ready = false
			p1_zhao1_ready = false
			return
		...
		if not up_pressed and not down_pressed and attack_skill2_next_attack_ready:
			play_action_if_available(p1, "atk4_extra", "atk4")
			attack_skill2_next_attack_ready = false
			return

	# 地面 S+J：原版 skill1() = down + attack，对应“砍技1”。
	# 注意：砍技1本体是反击准备，真正攻击动作是被打时转入“砍技1_HA”。
	if down_pressed and node_can_ground_action_safe(p1):
		node_play_action_safe(p1, "attack_skill1")
		return

	# 地面 W+J：原版 skill2() = up + attack，对应“砍技2”。
	if up_pressed and node_can_ground_action_safe(p1):
		attack_skill2_1_ready = false
		attack_skill2_next_attack_ready = false
		clear_p1_original_attack_chain_window()
		node_play_action_safe(p1, "attack_skill2")
	...
	if get_node_current_action_safe(p1) == "atk1" and get_node_frame_index_safe(p1) >= 7 and get_node_frame_index_safe(p1) < 18:
		node_play_action_safe(p1, "atk2")
		return

	if get_node_current_action_safe(p1) == "atk2" and get_node_frame_index_safe(p1) >= 8 and get_node_frame_index_safe(p1) < 16:
		node_play_action_safe(p1, "atk3")
		return

	if get_node_current_action_safe(p1) == "atk3" and get_node_frame_index_safe(p1) >= 11 and get_node_frame_index_safe(p1) < 26:
		node_play_action_safe(p1, "atk4")
		return

	if get_node_current_action_safe(p1) == "atk4_extra" and get_node_frame_index_safe(p1) >= 8 and get_node_frame_index_safe(p1) < 19:
		play_action_if_available(p1, "atk5", "atk4")
		return

	if node_can_ground_action_safe(p1):
		clear_p1_original_attack_chain_window()
		node_play_action_safe(p1, "atk1")


func can_start_p1_ground_bisha() -> bool:
	if p1 == null:
		return false
	if as_bool(p1.get("in_air")):
		return false
	if node_can_ground_action_safe(p1):
		return true
...
func handle_skill_input() -> void:
	...
	if get_node_in_air_safe(p1):
		# 原版 skillAIR 不等待 jump/fall 动作结束；airHitTimes 可用即可。
		if p1.has_method("can_original_air_action") and not as_bool(p1.call("can_original_air_action")):
			return
		p1_air_skill_ready = false
		p1_air_bisha_ready = false
		if p1.has_method("consume_original_air_action_limits"):
			p1.consume_original_air_action_limits()
		node_play_action_safe(p1, "air_skill")
		return
	...
	if p1_zhao1_ready:
		p1_zhao1_ready = false
		node_play_action_safe(p1, "skill1")
		return
	...
	if down_pressed:
		node_play_action_safe(p1, "skill2")
	elif up_pressed:
		node_play_action_safe(p1, "skill3")
	else:
		node_play_action_safe(p1, "skill1")
...
func handle_bisha_input() -> void:
	...
	# FighterMcCtrler.renderAirAction() 在空中每帧先判断 attackAIR / skillAIR / bishaAIR，
	# 不要求当前 jump/fall 动作已经播放结束；只要 airHitTimes 仍可用即可。
	if get_node_in_air_safe(p1):
		# 普通 Naruto(mc_0004) 没有“空中必杀”标签，不能像 Aizen 一样强行调用 super_air 或误耗气。
		if fighter_has_action(p1, "super_air"):
			if p1.has_method("can_original_air_action") and not as_bool(p1.call("can_original_air_action")):
				return
			...
			if p1.has_method("consume_original_air_action_limits"):
				p1.consume_original_air_action_limits()
			node_play_action_safe(p1, "super_air")
		return
	...
	if down_pressed:
		if try_use_p1_qi(300.0, "超必杀 / super_special"):
			...
			node_play_action_safe(p1, "super_special")
	elif up_pressed:
		if try_use_p1_qi(100.0, "上必杀 / super_up"):
			node_play_action_safe(p1, "super_up")
	else:
		if try_use_p1_qi(100.0, "普通必杀 / super"):
			node_play_action_safe(p1, "super")


func handle_p1_air_move(delta: float) -> void:
	# 原版 FighterMcCtrler.renderAirAction() 最后：if (_action.airMove) doAirMove();
	# doAirMove() 会在空中按 moveLEFT/moveRIGHT 水平移动，不切 walk 动画。
	if p1 == null or not as_bool(p1.get("in_air")):
		return
	if as_bool(p1.get("in_hitstun")) or ko_finished:
		return
...
func handle_p2_air_move(delta: float) -> void:
	# 同步 P2：原版 P2 输入同样走 FighterKeyCtrler.moveLEFT/moveRIGHT。
	if p2 == null or not as_bool(p2.get("in_air")):
		return
	if as_bool(p2.get("in_hitstun")) or ko_finished:
		return
	var moved: bool = false
	if p2_key_left_pressed() and not p2_key_right_pressed():
		p2.position.x -= move_speed * delta
		node_set_facing_safe(p2, -1)
		moved = true
	elif p2_key_right_pressed() and not p2_key_left_pressed():
		p2.position.x += move_speed * delta
		node_set_facing_safe(p2, 1)
		moved = true
	if moved:
...
func _on_p1_frame_event(action_name: String, event_data: Dictionary) -> void:
	...
	for call_data in event_data.get("calls", []):
		...
		# Step126：Naruto 超必杀原版是 fighter_ctrler.moveOnce(0,-150)，不是 setVelocity(0,-150)。
		# Step125 的解析器把它误归为 movement/move，导致视角被强制拉到空中并长时间无法落地。
		if str(raw_call.get("object", "")) == "fighter_ctrler" and str(raw_call.get("method", "")) == "moveOnce":
			move_fighter_once_by_original_args(p1, action_name, relative_frame, raw_call.get("args", []) as Array)
			continue
...
func _on_p2_frame_event(action_name: String, event_data: Dictionary) -> void:
	var relative_frame: int = int(event_data.get("relative_frame", -1))
	...
	for call_data in event_data.get("calls", []):
		...
		# Step126：Naruto 超必杀原版是 fighter_ctrler.moveOnce(0,-150)，不是 setVelocity(0,-150)。
		# P2 同步跳过 Step125 解析出来的错误 movement/move 语义。
		if str(raw_call.get("object", "")) == "fighter_ctrler" and str(raw_call.get("method", "")) == "moveOnce":
			move_fighter_once_by_original_args(p2, action_name, relative_frame, raw_call.get("args", []) as Array)
			continue

		if semantic.is_empty():
			handle_aizen_p2_raw_call_event(action_name, relative_frame, raw_call)
			continue

		handle_aizen_p2_semantic_event(action_name, relative_frame, semantic)


func handle_aizen_p2_raw_call_event(action_name: String, relative_frame: int, raw_call: Dictionary) -> void:
	...
	if object_name == "fighter_ctrler":
		match method_name:
			"moveOnce":
				move_fighter_once_by_original_args(p2, action_name, relative_frame, args)
			"justHit":
				handle_original_raw_just_hit_branch(p2, true, action_name, relative_frame, raw_call)
			"setDirectToTarget":
				face_p2_current_target(action_name, relative_frame)
			"setCross":
				var cross_enabled: bool = args.size() > 0 and as_bool(args[0])
				set_fighter_cross_enabled(p2, cross_enabled, action_name, relative_frame, "P2")
			_:
				pass
		return
	...
	if object_name == "mc_ctrler" and method_name == "moveTarget":
		...
		handle_original_move_target_event_for_actor(p2, p1, action_name, relative_frame, {"args_raw": move_args_raw}, "P2")
		return

	if object_name == "mc_ctrler" and method_name == "fire":
		start_original_naruto_fire_bullet(p2, args, str(raw_call.get("args_raw", "")), true, action_name, relative_frame)
		return

	if object_name == "mc_ctrler" and method_name == "setTouchFloor":
		handle_original_set_touch_floor_for_fighter(p2, args, action_name, relative_frame, true)
		return
	...
	if attacker_name == "bsmc":
		...
		play_p2_attacker_at_target("bsmc", raw_offset, 1.0)
		p2_active_original_hitbox = "bsmc"
	elif attacker_name == "zh3mc":
		play_p2_attacker_at_self("zh3mc", Vector2.ZERO, 1.0)
		p2_active_original_hitbox = "zh3mc"


func handle_aizen_p2_semantic_event(action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	var event_type: String = str(semantic.get("type", ""))

	match event_type:
		"timeline_sound":
			play_character_timeline_sound(p2, semantic, action_name, relative_frame)
		...
		"super_effect_start":
			...
			if bisha_effect != null:
				# Step80：原版 EffectCtrl.bisha(true) 先 cameraFocusOne(target)，再 doEffectById("bisha_super", target.x, target.y - 50)。
				# 当前 XG_cbs 在 HUD 层播放，所以先更新镜头，再把原版世界坐标换算到屏幕坐标。
				if is_super:
					begin_original_bisha_super_camera_focus(p2)
				var bisha_caster_origin: Vector2 = get_p2_original_effect_origin()
				var bisha_target_origin: Vector2 = get_p2_target_original_effect_origin()
				...
				bisha_effect.play_bisha(is_super, original_face_id, bisha_target_screen, get_node_facing(p2), bisha_current_target_screen, bisha_xg_screen, camera_zoom, 2)
		...
		"effect":
			...
			if effect_method == "shine":
				...
				play_original_dash_effect_for_fighter(p2, action_name, relative_frame)
		...
		"end_action":
			end_p2_shadow()
			clear_original_move_target_binding("p2 endAct")
			clear_original_catch_target("p2 endAct")
			if p2 != null and p2.has_method("set_steel_body"):
				p2.set_steel_body(false, false)
			if action_name == "jump_attack" or action_name == "air_skill" or action_name == "super_air":
				p2_air_skill_ready = false
				p2_air_bisha_ready = false
				if p2 != null and p2.has_method("unlock_original_action"):
					p2.unlock_original_action()

		"return_idle":
			end_p2_shadow()
			clear_original_move_target_binding("p2 idle")
			clear_original_catch_target("p2 idle")
			p2_air_skill_ready = false
			p2_air_bisha_ready = false
			p2_attack_skill2_1_ready = false
			p2_attack_skill2_next_attack_ready = false
			clear_p2_original_attack_chain_window()
			p2_zhao1_ready = false
			_reset_p2_active_attacker_state()
			if p2 != null and p2.has_method("clear_original_timeline_states"):
				p2.clear_original_timeline_states()
			if p2 != null and p2.has_method("return_to_neutral_from_timeline"):
				p2.return_to_neutral_from_timeline()
		...
		"hit_target_check":
			...
			if checker_name != "" and target_action != "":
				p2_pending_hit_target_checker = checker_name
				p2_pending_hit_target_action = target_action
				p2_pending_hit_target_enabled = true
		...
		"set_hurt_action":
			...
			if hurt_action != "" and p2 != null and p2.has_method("set_original_hurt_action"):
				p2.set_original_hurt_action(hurt_action)

		"add_qi":
			var qi_value: float = float(semantic.get("value", 0.0))
			if qi_value > 0.0 and p2 != null and p2.has_method("add_qi"):
				p2.add_qi(qi_value)
		...
		"enable_skill":
			...
			if enable_method == "setSkillAIR":
				...
				if skill2_action == "attack_skill2_1":
					p2_attack_skill2_1_ready = true
				else:
					p2_chain_skill2_ready = true
					try_execute_p2_original_chain_window_now()

		"enable_next_attack":
			var attack_label: String = str(semantic.get("target_label", ""))
			var attack_action: String = map_original_label_to_action(attack_label)
			if attack_action != "":
				p2_next_attack_action = attack_action
				p2_next_attack_ready = true
				if attack_action == "atk4_extra":
					p2_attack_skill2_next_attack_ready = true
				try_execute_p2_original_chain_window_now()
		...
		"set_cross":
			...
			set_fighter_cross_enabled(p2, cross_enabled_sem, action_name, relative_frame, "P2")
		...
		"air_move":
			...
			if p2 != null and p2.has_method("set_air_move_enabled"):
				p2.set_air_move_enabled(air_move_enabled)

		"if_last_hit_play":
			handle_original_just_hit_to_play_for_fighter(p2, true, action_name, relative_frame, semantic)

		"stop_animation":
			if p2 != null and p2.has_method("stop_original_animation"):
				p2.stop_original_animation()

		"move_target":
			# 原版 FighterMcCtrler.moveTarget(params) 绑定的是当前施法者的 currentTarget。
			# P2 触发时目标应是 P1；旧实现只给 P1 处理，或全局错误绑定到 P2，
			# 会导致 P2 自己被 original_move_target_bound 锁住，从而特定技能后无法控制。
			handle_original_move_target_event_for_actor(p2, p1, action_name, relative_frame, semantic, "P2")
		...
		"teleport_or_move_to_target":
			...
			move_fighter_to_target(p2, p1, offset_x_value, offset_y_value, auto_turn)

		"apply_gravity":
			var apply_g: bool = semantic.get("enabled", true) == true
			if p2 != null and p2.has_method("set_apply_gravity_enabled"):
				p2.set_apply_gravity_enabled(apply_g)

		"movement":
			handle_original_movement_event_for_fighter(p2, action_name, relative_frame, semantic)

		"dash_or_stop":
			handle_original_dash_or_stop_event_for_fighter(p2, action_name, relative_frame, semantic)

		"add_attacker":
			var attacker_name: String = str(semantic.get("attacker", ""))
			if start_character_attacker_for_fighter(p2, attacker_name, true):
				pass
			elif attacker_name == "bsmc":
				play_p2_attacker_at_target("bsmc", BSMC_ORIGINAL_TARGET_OFFSET, 1.0)
				p2_active_original_hitbox = "bsmc"
			elif attacker_name == "zh3mc":
				play_p2_attacker_at_self("zh3mc", Vector2.ZERO, 1.0)
				p2_active_original_hitbox = "zh3mc"
...
func get_character_action_instance_local_rect(fighter: Variant, action_name: String, instance_name: String) -> Rect2:
	var id: String = get_fighter_character_id(fighter, "aizen")
	if not character_attack_manifests.has(id):
		load_character_attack_manifest(id)
	var manifest: Dictionary = character_attack_manifests.get(id, {}) as Dictionary
	var actions: Dictionary = manifest.get("actions", {}) as Dictionary
	var entries: Array = actions.get(action_name, []) as Array
	for item_v in entries:
		if typeof(item_v) != TYPE_DICTIONARY:
			continue
		var item: Dictionary = item_v as Dictionary
		if str(item.get("instance", "")) != instance_name:
			continue
		var r: Array = item.get("rect", []) as Array
		if r.size() < 4:
			continue
		return Rect2(Vector2(float(r[0]), float(r[1])), Vector2(float(r[2]), float(r[3])))
	return Rect2()


func get_original_naruto_fire_spec(mc_name: String, args_raw: String) -> Dictionary:
	# 原版 mc_0004：
	#   fire("zh1atm",{x:{start:15,hit:3},hold:1,hits:3})  -> U / zh1 / Bullet
	#   fire("zh2atm",{x:{start:10,hit:5},hold:1.5,hits:5,hp:200}) -> S+U / zh2 / Bullet
	var spec: Dictionary = {}
	if mc_name == "zh1atm":
		spec = {"hit":"zh1", "action":"skill1", "start":15.0, "hit_speed":3.0, "hold":1.0, "hits":3}
	elif mc_name == "zh2atm":
		spec = {"hit":"zh2", "action":"skill2", "start":10.0, "hit_speed":5.0, "hold":1.5, "hits":5}
	else:
		return {}
	# 保留从 raw 中读取的能力，避免后续重新导出参数变化。
	var start_value: float = parse_named_number_after_marker(args_raw, "start:", float(spec.get("start", 0.0)))
	var hit_value: float = parse_named_number_after_marker(args_raw, "hit:", float(spec.get("hit_speed", 0.0)))
	var hold_value: float = parse_named_number_after_marker(args_raw, "hold:", float(spec.get("hold", 0.0)))
	var hits_value: float = parse_named_number_after_marker(args_raw, "hits:", float(spec.get("hits", 1)))
	spec["start"] = start_value
	spec["hit_speed"] = hit_value
	spec["hold"] = hold_value
	spec["hits"] = int(hits_value)
	return spec


func parse_named_number_after_marker(text: String, marker: String, fallback: float) -> float:
	var pos: int = text.find(marker)
	if pos < 0:
		return fallback
	var start: int = pos + marker.length()
	var end: int = start
	...
	var value_text: String = text.substr(start, end - start).strip_edges()
	if value_text == "":
		return fallback
	return float(value_text)


func start_original_naruto_fire_bullet(fighter: Variant, args: Array, args_raw: String, is_p2_actor: bool, action_name: String, relative_frame: int) -> bool:
	if fighter == null or args.is_empty():
		return false
	if get_fighter_character_id(fighter, "") != "naruto":
		return false
	var mc_name: String = str(args[0])
	var spec: Dictionary = get_original_naruto_fire_spec(mc_name, args_raw)
	if spec.is_empty():
		return false
	var bullet_action: String = str(spec.get("action", action_name))
	var local_rect: Rect2 = get_character_action_instance_local_rect(fighter, bullet_action, mc_name)
	if local_rect.size == Vector2.ZERO:
		return false
	var facing_dir: int = get_node_facing(fighter)
	var origin: Vector2 = get_node_ground_pos(fighter, P1_START_POS if not is_p2_actor else P2_START_POS)
	var bullet: Dictionary = {
		"owner": fighter,
		"action": bullet_action,
		"mc": mc_name,
		"hit": str(spec.get("hit", "")),
		"rect": local_rect,
		"pos": origin,
		"facing": facing_dir,
		"speed_x": float(spec.get("start", 0.0)) * float(facing_dir),
		"hit_speed_x": float(spec.get("hit_speed", 0.0)) * float(facing_dir),
		"hold_left": float(spec.get("hold", 1.0)),
		"hits_left": int(spec.get("hits", 1)),
		"frame_accum": 0.0,
		"last_hit_tick": -99999,
		"debug_label": "Naruto fire " + mc_name
	}
	if is_p2_actor:
		p2_original_bullets.append(bullet)
	else:
		p1_original_bullets.append(bullet)
	print("Naruto 原版 fire Bullet 启动 actor=", "P2" if is_p2_actor else "P1", " action=", action_name, " frame=", relative_frame, " mc=", mc_name, " spec=", spec, " rect=", local_rect)
	return true


func update_original_naruto_bullets(delta: float) -> void:
	update_original_naruto_bullet_list(p1_original_bullets, false, delta)
	update_original_naruto_bullet_list(p2_original_bullets, true, delta)


func update_original_naruto_bullet_list(bullets: Array, is_p2_actor: bool, delta: float) -> void:
	for i: int in range(bullets.size() - 1, -1, -1):
		var bullet: Dictionary = bullets[i] as Dictionary
		var owner: Variant = bullet.get("owner", null)
		if owner == null or not is_instance_valid(owner):
			bullets.remove_at(i)
			continue
		var hold_left: float = float(bullet.get("hold_left", 0.0)) - delta
		bullet["hold_left"] = hold_left
		if hold_left <= 0.0 or int(bullet.get("hits_left", 0)) <= 0:
			bullets.remove_at(i)
			continue
		var frame_step: float = ORIGINAL_GAME_RENDER_FPS * delta
		bullet["frame_accum"] = float(bullet.get("frame_accum", 0.0)) + frame_step
		var pos: Vector2 = bullet.get("pos", Vector2.ZERO) as Vector2
		pos.x += float(bullet.get("speed_x", 0.0)) * ORIGINAL_SPEED_PLUS_DEFAULT * frame_step
		bullet["pos"] = pos
		bullets[i] = bullet
		var local_rect: Rect2 = bullet.get("rect", Rect2()) as Rect2
		var facing_dir: int = int(bullet.get("facing", 1))
		var world_rect: Rect2 = make_directed_rect(pos, local_rect, facing_dir)
		var target: Variant = p1 if is_p2_actor else p2
		var hurt_rect: Rect2 = get_p1_hurt_rect() if is_p2_actor else get_target_hurt_rect()
		if attack_debug_enabled:
			if is_p2_actor:
				draw_p2_attack_box(world_rect, "P2 " + str(bullet.get("debug_label", "bullet")))
			else:
				draw_original_attack_box(world_rect, str(bullet.get("debug_label", "bullet")))
		if target == null or not world_rect.intersects(hurt_rect, true):
			continue
		var tick: int = int(floor(float(bullet.get("frame_accum", 0.0)) / ORIGINAL_NARUTO_PROJECTILE_REHIT_FRAMES))
		if tick == int(bullet.get("last_hit_tick", -99999)):
			continue
		var hit_id: String = str(bullet.get("hit", ""))
		var hit_vo: Dictionary = get_hit_vo_for_attacker(owner, hit_id)
		var hit_rect: Rect2 = world_rect.intersection(hurt_rect)
		if hit_rect.size == Vector2.ZERO:
			hit_rect = world_rect
		var hit_result: Dictionary = {}
		if target != null and target.has_method("apply_original_hit"):
			hit_result = target.apply_original_hit(hit_vo, get_node_facing(owner))
		if str(hit_result.get("result", "")) == "knock_fly" and target != null and target.has_method("force_original_hurttype1_knock_fly_if_needed"):
			target.call("force_original_hurttype1_knock_fly_if_needed", hit_vo, get_node_facing(owner))
		if str(hit_result.get("result", "")) != "ignored":
			bullet["last_hit_tick"] = tick
			bullet["hits_left"] = max(0, int(bullet.get("hits_left", 1)) - 1)
			bullet["speed_x"] = float(bullet.get("hit_speed_x", bullet.get("speed_x", 0.0)))
			if is_p2_actor:
				p2_last_hit_vo_id = hit_id
				p2_last_hit_action = str(owner.get("current_action")) if owner != null else ""
			else:
				p1_last_hit_vo_id = hit_id
				p1_last_hit_action = str(owner.get("current_action")) if owner != null else ""
			bullets[i] = bullet
			after_original_bullet_hit(owner, target, hit_vo, hit_result, hit_rect, is_p2_actor)
			if int(bullet.get("hits_left", 0)) <= 0:
				bullets.remove_at(i)


func after_original_bullet_hit(owner: Variant, target: Variant, hit_vo: Dictionary, hit_result: Dictionary, hit_rect: Rect2, is_p2_actor: bool) -> void:
	var result: String = str(hit_result.get("result", ""))
	play_original_hit_feedback(hit_vo, hit_rect, result, get_node_facing(owner), target)
	if result != "ignored":
		add_qi_on_hit(owner, target, hit_vo, "bullet")
...
func handle_original_set_touch_floor_for_fighter(fighter: Variant, args: Array, action_name: String, relative_frame: int, is_p2_actor: bool) -> void:
	if fighter == null or args.is_empty():
		return
	var label_name: String = str(args[0])
	var break_act: bool = true
	if args.size() > 1:
		break_act = as_bool(args[1])
	var target_action: String = map_original_label_to_action(label_name)
	if target_action == "":
		target_action = label_name
	if fighter.has_method("set_original_touch_floor_action"):
		fighter.set_original_touch_floor_action(target_action, break_act)
	else:
		fighter.set_meta("original_touch_floor_action", target_action)
		fighter.set_meta("original_touch_floor_break_act", break_act)
	print("原版 setTouchFloor 触发 actor=", "P2" if is_p2_actor else "P1", " action=", action_name, " frame=", relative_frame, " -> ", target_action, " breakAct=", break_act)


func handle_original_raw_just_hit_branch(fighter: Variant, is_p2_actor: bool, action_name: String, relative_frame: int, raw_call: Dictionary) -> void:
	var raw_text: String = str(raw_call.get("args_raw", ""))
	if raw_text.find("bs1") < 0 or raw_text.find("必杀_0") < 0:
		return
	var last_id: String = p2_last_hit_vo_id if is_p2_actor else p1_last_hit_vo_id
	var last_action: String = p2_last_hit_action if is_p2_actor else p1_last_hit_action
	var matched: bool = last_id == "bs1" and last_action == action_name
	print("Naruto 原版 justHit(bs1) 分支检查 actor=", "P2" if is_p2_actor else "P1", " matched=", matched, " last=", last_id, "/", last_action)
	if not matched and fighter != null and fighter.has_method("play_action") and fighter_has_action(fighter, "super_0"):
		fighter.play_action("super_0")
...
func handle_aizen_raw_call_event(action_name: String, relative_frame: int, raw_call: Dictionary) -> void:
	...
	if object_name == "fighter_ctrler":
		match method_name:
			...
			"justHit":
				handle_original_raw_just_hit_branch(p1, false, action_name, relative_frame, raw_call)
			"setDirectToTarget":
	...
	if object_name == "mc_ctrler" and method_name == "fire":
		start_original_naruto_fire_bullet(p1, args, str(raw_call.get("args_raw", "")), false, action_name, relative_frame)
		return

	if object_name == "mc_ctrler" and method_name == "setTouchFloor":
		handle_original_set_touch_floor_for_fighter(p1, args, action_name, relative_frame, false)
		return
	...
	if start_character_attacker_for_fighter(p1, attacker_name, false):
		return
...
func handle_aizen_semantic_event(action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	...
	match event_type:
		"timeline_sound":
			play_character_timeline_sound(p1, semantic, action_name, relative_frame)
		...
		"super_effect_start":
			...
			if bisha_effect != null:
				...
				# Step80：原版 S+I / bisha_super 会先 cameraFocusOne(target.getDisplay())，再 doEffectById("bisha_super", target.x, target.y - 50)。
				# 当前 XG_cbs 在屏幕层播放，因此先让镜头进入单人特写，再换算 XG_cbs 和脸图位置。
				if is_super:
					begin_original_bisha_super_camera_focus(p1)
				...
				var bisha_xg_screen: Vector2 = world_to_screen(bisha_caster_origin + Vector2(0.0, -50.0))
				begin_original_bisha_stage_mode(is_super)
				bisha_effect.play_bisha(is_super, original_face_id, bisha_target_screen, get_node_facing(p1), bisha_current_target_screen, bisha_xg_screen, camera_zoom, 1)
		...
		"enable_skill":
			...
			if enable_method == "setSkillAIR":
				...
				if skill2_action == "attack_skill2_1":
					...
					p1_chain_skill2_ready = true
					try_execute_p1_original_chain_window_now()

		"enable_next_attack":
			# 原版 setAttack(label)：把下一次普通 J 的动作切到对应 label；
			# 输入缓冲仍在时，应同帧立刻消费，不等动作播到末尾。
			var attack_label: String = str(semantic.get("target_label", ""))
			var attack_action: String = map_original_label_to_action(attack_label)
			print("原版 setAttack 触发 action=", action_name, " frame=", relative_frame, " target=", attack_label, " -> ", attack_action)
			if attack_action != "":
				p1_next_attack_action = attack_action
				p1_next_attack_ready = true
				if attack_action == "atk4_extra":
...
func start_fighter_shadow(fighter: Variant, r: int = 0, g: int = 0, b: int = 0) -> void:
	# FighterEffectCtrler.shadow(r,g,b) -> EffectCtrler.startShadow(_targetDisplay,r,g,b)。
	if shadow_effects == null or fighter == null:
		return
	shadow_effects.start_shadow(fighter, r, g, b)


func start_p1_shadow(r: int = 0, g: int = 0, b: int = 0) -> void:
	start_fighter_shadow(p1, r, g, b)
...
func end_p2_shadow() -> void:
	# 当前 ShadowEffectLayer 是统一层，P2 与 P1 一样只停止继续新增残影。
	if shadow_effects == null:
		return
	shadow_effects.end_shadow()
...
func map_original_label_to_action(label_name: String) -> String:
	# 通用原版标签 -> Godot action 映射。Aizen / Naruto 都从各自 XFL 标签读取，
	# 不能把 Naruto 强行映射到 Aizen 专属动作；这里只做标签名到通用 action name 的翻译。
	var label_map: Dictionary = {
		...
		"砍5": "atk5",
		"砍技1": "attack_skill1",
		"砍技1_CHK": "attack_skill1_check",
		"砍技1_HA": "attack_skill1_extra",
		"砍技1_1": "attack_skill1_1",
		"砍技2": "attack_skill2",
		"砍技2_CHK": "attack_skill2_check",
		"砍技2_1": "attack_skill2_1",
		"砍技2_HIT": "attack_skill2_hit",
		"招1": "skill1",
		"招1_CHK": "skill1_check",
		"招2": "skill2",
		"招2_CHK": "skill2_check",
		...
		"必杀_CHK": "super_check",
		"上必杀": "super_up",
		"上必杀_HIT": "super_up_hit",
		"空中必杀": "super_air",
		"空中必杀_L": "super_air_l",
		"空中必杀_TF": "super_air_tf",
		"超必杀": "super_special",
		"超必杀_L": "super_special_l",
		"超必杀_TF": "super_special_tf",
		"防御": "defend",
		"防御恢复": "defend_recover",
		"被打": "hurt",
		"击飞": "knock_fly",
	...
	return str(label_map.get(label_name, ""))
...
func get_character_attacker_manifest_for_fighter(fighter: Variant) -> Dictionary:
	if fighter == null:
		return {}
	var id: String = get_fighter_character_id(fighter, "aizen")
	if id == "aizen":
		return {}
	if not character_attacker_manifests.has(id):
		load_character_attacker_manifest(id)
	return character_attacker_manifests.get(id, {}) as Dictionary


func get_character_attacker_data(fighter: Variant, attacker_name: String) -> Dictionary:
	var manifest: Dictionary = get_character_attacker_manifest_for_fighter(fighter)
	var attackers: Dictionary = manifest.get("attackers", {}) as Dictionary
	return attackers.get(attacker_name, {}) as Dictionary


func get_active_attacker_runtime_frame(is_p2_actor: bool) -> int:
	var start_ms: int = p2_active_attacker_start_ms if is_p2_actor else active_attacker_start_ms
	if start_ms <= 0:
		return 0
	var elapsed_ms: int = maxi(0, Time.get_ticks_msec() - start_ms)
	return int(floor(float(elapsed_ms) / (1000.0 / ORIGINAL_FPS)))


func get_character_attacker_duration(fighter: Variant, attacker_name: String) -> int:
	var data: Dictionary = get_character_attacker_data(fighter, attacker_name)
	return int(data.get("duration", 0))


func get_character_attacker_hit_window(fighter: Variant, attacker_name: String, frame: int) -> Dictionary:
	var data: Dictionary = get_character_attacker_data(fighter, attacker_name)
	if data.is_empty():
		return {}
	var child_hits: Array = data.get("child_hits", []) as Array
	for item_v in child_hits:
		if typeof(item_v) != TYPE_DICTIONARY:
			continue
		var item: Dictionary = item_v as Dictionary
		var frame_start: int = int(item.get("frame_start", -99999))
		var frame_end: int = int(item.get("frame_end", -99999))
		if frame < frame_start or frame > frame_end:
			continue
		var r: Array = item.get("rect", []) as Array
		if r.size() < 4:
			continue
		var rect := Rect2(Vector2(float(r[0]), float(r[1])), Vector2(float(r[2]), float(r[3])))
		var hit_id: String = str(item.get("hit", ""))
		var label_text: String = str(item.get("label", attacker_name + " / " + hit_id))
		var tick: int = int(floor(float(frame - frame_start) / ORIGINAL_REHIT_GAP_FRAMES))
		var key: String = str(item.get("key", attacker_name + ":" + hit_id)) + ":" + str(tick)
		return {"rect": rect, "label": label_text, "hit": hit_id, "key": key}
	return {}


func start_character_attacker_for_fighter(fighter: Variant, attacker_name: String, is_p2_actor: bool) -> bool:
	if fighter == null or attacker_name == "":
		return false
	var id: String = get_fighter_character_id(fighter, "aizen")
	if id == "aizen":
		return false
	...
	if data.is_empty():
		return false
	var matrix: Dictionary = data.get("instance_matrix", {}) as Dictionary
	var offset := Vector2(float(matrix.get("tx", 0.0)), float(matrix.get("ty", 0.0)))
	var facing_dir: int = int(fighter.get("facing"))
	var origin := Vector2.ZERO
	if fighter is Node2D:
		origin = (fighter as Node2D).position + Vector2(offset.x * float(facing_dir), offset.y)
		# Naruto W+I 原版 addAttacker("sbsmc",{x:{moveToTarget:true,offset:0},y:{moveToTarget:false,offset:0},applyG:true})。
		# FighterAttacker.init() 会把 X 直接放到目标 x，Y 留在施法者 y，而不是使用 sbsmc 的静态 Matrix tx=56.55/ty=-6.3。
		if get_fighter_character_id(fighter, "") == "naruto" and attacker_name == "sbsmc":
			var target_node: Variant = p1 if is_p2_actor else p2
			if target_node != null and target_node is Node2D:
				origin.x = (target_node as Node2D).position.x
				origin.y = (fighter as Node2D).position.y
	if is_p2_actor:
		p2_active_original_hitbox = attacker_name
		p2_active_attacker_effect_name = attacker_name
		p2_active_attacker_original_origin = origin
		p2_active_attacker_facing = facing_dir
		p2_active_attacker_start_ms = Time.get_ticks_msec()
		p2_test_attack_has_hit = false
		p2_test_attack_key = ""
	else:
		active_original_hitbox = attacker_name
		active_attacker_effect_name = attacker_name
		active_attacker_original_origin = origin
		active_attacker_facing = facing_dir
		active_attacker_start_ms = Time.get_ticks_msec()
		active_attacker_has_hit = false
		active_attack_key = ""

	var effect_player = p2_effect if is_p2_actor else p1_effect
	if effect_player != null and effect_player.has_method("play_effect"):
		# Step119：现在 naruto_effect_manifest 已有正确 sbsmc/cbsmc/kj2mc/kj2mc2 帧，
		# 可见特效也按原版 addAttacker instance matrix 的 origin 播放。
		effect_player.call("play_effect", attacker_name, origin, facing_dir, 1.0, false)
	print("Naruto addAttacker 启动: ", attacker_name, " char=", id, " origin=", origin, " owner=", data.get("owner_action", ""))
	return true


func get_character_attack_hit_window(char_id: String, action_name: String, frame: int) -> Dictionary:
	var id: String = char_id.to_lower()
	if id != "aizen":
		# Naruto 招1/招2 原版由 mc_ctrler.fire() 生成 Bullet；不能再把同一个 zh1atm/zh2atm 当主时间轴攻击面重复命中。
		if id == "naruto" and (action_name == "skill1" or action_name == "skill2"):
			return {}
		if not character_attack_manifests.has(id):
			load_character_attack_manifest(id)
		var manifest: Dictionary = character_attack_manifests.get(id, {}) as Dictionary
		...
		for item_v in entries:
			...
			if r.size() < 4:
				continue
			var rect := Rect2(Vector2(float(r[0]), float(r[1])), Vector2(float(r[2]), float(r[3])))
			if id == "naruto" and action_name == "jump_attack":
				# 原版 tkatm 是随跳砍身体注册点走的攻击面；当前 normalized PNG 的 Node2D 注册点偏到画布中心，
				# 直接使用未补偿 rect 会表现为 K+J 判定距离过短。这里只做注册点级扩展，不改 HitVO 伤害。
				rect.position.x -= ORIGINAL_NARUTO_JUMP_ATTACK_REACH_PAD_X
				rect.size.x += ORIGINAL_NARUTO_JUMP_ATTACK_REACH_PAD_X * 2.0
				rect.position.y -= ORIGINAL_NARUTO_JUMP_ATTACK_REACH_PAD_Y
				rect.size.y += ORIGINAL_NARUTO_JUMP_ATTACK_REACH_PAD_Y * 2.0
			var hit_id: String = str(item.get("hit", ""))
			var label_text: String = str(item.get("label", action_name + " / " + hit_id))
			var key: String = str(item.get("key", id + ":" + action_name + ":" + hit_id + ":" + str(frame_start) + "-" + str(frame_end)))
			return {"rect": rect, "label": label_text, "hit": hit_id, "key": key}
	# Aizen fallback：保留原已校准硬编码窗口。
	return get_main_attack_hit_window(action_name, frame)
...
func update_main_attack_box() -> bool:
	...
	var data: Dictionary = get_character_attack_hit_window(get_fighter_character_id(p1, "aizen"), get_node_current_action_safe(p1), get_node_frame_index_safe(p1))
	if data.is_empty():
		return false
	...
	var world_rect: Rect2 = make_directed_rect(get_node_ground_pos(p1, P1_START_POS), local_rect, get_node_facing(p1))
	active_original_hitbox = "main_attack"
	set_current_attack_box(world_rect, label_text, hit_vo_id, attack_key)
	return true


func get_character_checker_rect_for_fighter(fighter: Variant, checker_name: String) -> Rect2:
	if fighter == null:
		return Rect2()
	var id: String = get_fighter_character_id(fighter, "aizen")
	if id == "aizen":
	...
	if not character_attack_manifests.has(id):
		load_character_attack_manifest(id)
	var manifest: Dictionary = character_attack_manifests.get(id, {}) as Dictionary
	var checkers: Dictionary = manifest.get("checkers", {}) as Dictionary
	if not checkers.has(checker_name):
		return Rect2()
	var item: Dictionary = checkers.get(checker_name, {}) as Dictionary
	var r: Array = item.get("rect", []) as Array
	if r.size() < 4:
		return Rect2()
	return make_directed_rect(
		(fighter as Node2D).position,
		Rect2(Vector2(float(r[0]), float(r[1])), Vector2(float(r[2]), float(r[3]))),
		int(fighter.get("facing"))
	)


func get_character_checker_owner_action(fighter: Variant, checker_name: String) -> String:
	if fighter == null:
		return ""
	var id: String = get_fighter_character_id(fighter, "aizen")
	if id == "aizen":
	...
	if not checkers.has(checker_name):
		return ""
	var item: Dictionary = checkers.get(checker_name, {}) as Dictionary
	return str(item.get("owner_action", ""))


func get_hit_target_checker_rect(checker_name: String) -> Rect2:
	var generic_rect: Rect2 = get_character_checker_rect_for_fighter(p1, checker_name)
	if generic_rect.size != Vector2.ZERO:
		return generic_rect
	if checker_name == "kj2_chkm":
		return make_directed_rect(get_node_ground_pos(p1, P1_START_POS), KJ2_CHKM_RECT, get_node_facing(p1))
	if checker_name == "zh2_chkm":
		return make_directed_rect(get_node_ground_pos(p1, P1_START_POS), ZH2_CHKM_RECT, get_node_facing(p1))
	return Rect2()
...
func update_pending_hit_target_check() -> void:
	...
	var expected_action: String = get_character_checker_owner_action(p1, pending_hit_target_checker)
	if expected_action == "":
	...
	if expected_action != "" and get_node_current_action_safe(p1) != expected_action:
...
func update_original_attack_box() -> void:
	...
	if active_original_hitbox != "bsmc" and active_original_hitbox != "zh3mc":
		var attacker_frame: int = get_active_attacker_runtime_frame(false)
		var data: Dictionary = get_character_attacker_hit_window(p1, active_original_hitbox, attacker_frame)
		if data.is_empty():
			var dur: int = get_character_attacker_duration(p1, active_original_hitbox)
			if dur > 0 and attacker_frame > dur + 2:
				active_original_hitbox = ""
				clear_original_attack_box()
			else:
				hide_current_attack_box_for_empty_frame()
			return
		var local_rect: Rect2 = data.get("rect", Rect2())
		var world_rect := make_directed_rect(active_attacker_original_origin, local_rect, active_attacker_facing)
		set_current_attack_box(world_rect, str(data.get("label", active_original_hitbox)), str(data.get("hit", "")), str(data.get("key", active_original_hitbox)))
		return
...
func get_node_facing(node) -> int:
	# Step122：基于当前上传项目的完整安全兜底。
	# 如果角色 manifest 缺失，p1/p2 会退回 Label；Label 没有 facing/current_action/frame_index。
	if node != null:
		if node is Node2D:
	...
	if p1 != null and p1 is Node2D:
		var p1_v: Variant = (p1 as Node2D).get("facing")
		if typeof(p1_v) == TYPE_INT or typeof(p1_v) == TYPE_FLOAT:
			return -1 if int(p1_v) >= 0 else 1

	return -1


func as_bool(value: Variant) -> bool:
	# Godot 4.6 stable mono 环境不接受 bool(value) 构造写法。
	# 这里统一做安全布尔转换，避免 Nonexistent 'bool' constructor。
	if typeof(value) == TYPE_BOOL:
		return value
	if typeof(value) == TYPE_INT or typeof(value) == TYPE_FLOAT:
...
func get_node_current_action_safe(node, fallback: String = "") -> String:
	if node != null and is_instance_valid(node) and node is Object:
		var v: Variant = (node as Object).get("current_action")
		if typeof(v) == TYPE_STRING:
			return str(v)
	return fallback


func get_node_frame_index_safe(node, fallback: int = 0) -> int:
	if node != null and is_instance_valid(node) and node is Object:
		var v: Variant = (node as Object).get("frame_index")
		if typeof(v) == TYPE_INT or typeof(v) == TYPE_FLOAT:
			return int(v)
	return fallback


func get_node_in_air_safe(node) -> bool:
	if node != null and is_instance_valid(node) and node is Object:
		var v: Variant = (node as Object).get("in_air")
		return as_bool(v)
	return false


func node_can_ground_action_safe(node) -> bool:
	return node != null and is_instance_valid(node) and node.has_method("can_ground_action") and as_bool(node.call("can_ground_action"))


func node_play_action_safe(node, action_name: String) -> void:
	if node != null and is_instance_valid(node) and node.has_method("play_action"):
		node.call("play_action", action_name)


func node_set_facing_safe(node, new_facing: int) -> void:
	if node != null and is_instance_valid(node) and node.has_method("set_facing"):
		node.call("set_facing", new_facing)
...
func get_p2_hit_target_checker_rect(checker_name: String) -> Rect2:
	var generic_rect: Rect2 = get_character_checker_rect_for_fighter(p2, checker_name)
	if generic_rect.size != Vector2.ZERO:
		return generic_rect
	if checker_name == "kj2_chkm":
		return make_directed_rect(get_node_ground_pos(p2, P2_START_POS), KJ2_CHKM_RECT, get_node_facing(p2))
	if checker_name == "zh2_chkm":
		return make_directed_rect(get_node_ground_pos(p2, P2_START_POS), ZH2_CHKM_RECT, get_node_facing(p2))
	return Rect2()


func update_p2_pending_hit_target_check() -> void:
	if not p2_pending_hit_target_enabled:
		return
	if p1 == null or p2 == null:
		return

	var expected_action: String = get_character_checker_owner_action(p2, p2_pending_hit_target_checker)
	if expected_action == "":
		if p2_pending_hit_target_checker == "kj2_chkm":
			expected_action = "attack_skill2"
		elif p2_pending_hit_target_checker == "zh2_chkm":
			expected_action = "skill2"

	if expected_action != "" and get_node_current_action_safe(p2) != expected_action:
		p2_pending_hit_target_enabled = false
		p2_pending_hit_target_checker = ""
		p2_pending_hit_target_action = ""
		return

	var checker_rect: Rect2 = get_p2_hit_target_checker_rect(p2_pending_hit_target_checker)
	if checker_rect.size == Vector2.ZERO:
		return
	if attack_debug_enabled:
		draw_p2_attack_box(checker_rect, "P2 CHECK " + p2_pending_hit_target_checker + " -> " + p2_pending_hit_target_action)

	var hurt_rect: Rect2 = get_p1_hurt_rect()
	if not checker_rect.intersects(hurt_rect, true):
		return

	print("原版 P2 setHitTarget 命中：", p2_pending_hit_target_checker, " -> ", p2_pending_hit_target_action)
	p2_pending_hit_target_enabled = false
	p2_pending_hit_target_checker = ""
	var next_action: String = p2_pending_hit_target_action
	p2_pending_hit_target_action = ""
	node_play_action_safe(p2, next_action)


func set_p2_current_attack_key(key: String) -> void:
...
func update_p2_test_attack_box() -> void:
	# Step60：从原来“P2 debug atk1”升级为正式 P2 攻击检测。
	# 判定矩形继续使用 Step43~57 已按 aizen mc_023.xml 校准的 get_main_attack_hit_window()。
	if p2 == null or p1 == null or not p2.has_method("play_action"):
		clear_p2_attack_box()
		return

	var action_name: String = str(p2.get("current_action"))
	var frame: int = int(p2.get("frame_index"))
	var data: Dictionary = get_character_attack_hit_window(get_fighter_character_id(p2, "aizen"), action_name, frame)
	var attack_rect: Rect2 = Rect2()
	var label_text: String = ""
	var hit_vo_id: String = ""
	var attack_key: String = ""

	if not data.is_empty():
		var local_rect: Rect2 = data.get("rect", Rect2())
		label_text = "P2 " + str(data.get("label", ""))
		hit_vo_id = str(data.get("hit", ""))
		attack_key = "p2:" + str(data.get("key", label_text + ":" + hit_vo_id))
		attack_rect = make_directed_rect(get_node_ground_pos(p2, P2_START_POS), local_rect, get_node_facing(p2))
		p2_active_original_hitbox = "main_attack"
	elif p2_active_original_hitbox == "bsmc" and p2_effect != null and p2_effect.playing and p2_effect.current_effect == "bsmc":
		if p2_effect.frame_index >= 2 and p2_effect.frame_index <= 48:
			var bs1_tick: int = int(floor(float(p2_effect.frame_index - 2) / ORIGINAL_REHIT_GAP_FRAMES))
			attack_rect = make_directed_rect(p2_active_attacker_original_origin, BS1ATM_RECT, p2_active_attacker_facing)
			label_text = "P2 bsmc / bs1atm"
			hit_vo_id = "bs1"
			attack_key = "p2:bsmc:bs1:" + str(bs1_tick)
		elif p2_effect.frame_index >= 49 and p2_effect.frame_index <= 52:
			attack_rect = make_directed_rect(p2_active_attacker_original_origin, BSATM_RECT, p2_active_attacker_facing)
			label_text = "P2 bsmc / bsatm"
			hit_vo_id = "bs"
			attack_key = "p2:bsmc:bs"
		else:
			return
	elif p2_active_original_hitbox == "zh3mc" and p2_effect != null and p2_effect.playing and p2_effect.current_effect == "zh3mc":
		attack_rect = make_directed_rect(p2_active_attacker_original_origin, ZH3ATM_RECT, p2_active_attacker_facing)
		label_text = "P2 zh3mc / zh3atm"
		hit_vo_id = "zh3"
		attack_key = "p2:zh3mc:zh3"
	elif p2_active_original_hitbox != "" and p2_active_original_hitbox != "main_attack" and p2_active_original_hitbox != "bsmc" and p2_active_original_hitbox != "zh3mc":
		var attacker_frame: int = get_active_attacker_runtime_frame(true)
		var attacker_data: Dictionary = get_character_attacker_hit_window(p2, p2_active_original_hitbox, attacker_frame)
		if attacker_data.is_empty():
			var dur: int = get_character_attacker_duration(p2, p2_active_original_hitbox)
			if dur > 0 and attacker_frame > dur + 2:
				p2_active_original_hitbox = ""
				clear_p2_attack_box()
			return
		var local_rect: Rect2 = attacker_data.get("rect", Rect2())
		attack_rect = make_directed_rect(p2_active_attacker_original_origin, local_rect, p2_active_attacker_facing)
		label_text = "P2 " + str(attacker_data.get("label", p2_active_original_hitbox))
		hit_vo_id = str(attacker_data.get("hit", ""))
		attack_key = "p2:" + str(attacker_data.get("key", p2_active_original_hitbox))
	else:
		if p2_active_original_hitbox == "main_attack":
			p2_active_original_hitbox = ""
		clear_p2_attack_box()
		return

	if attack_rect.size == Vector2.ZERO or hit_vo_id == "":
		return

	set_p2_current_attack_key(attack_key)
	draw_p2_attack_box(attack_rect, label_text + " -> HitVO:" + hit_vo_id)
	...
	var hit_vo: Dictionary = get_hit_vo_for_attacker(p2, hit_vo_id)
	var hit_result: Dictionary = {}
	if p1 != null and p1.has_method("apply_original_hit"):
		hit_result = p1.apply_original_hit(hit_vo, get_node_facing(p2))
	if str(hit_result.get("result", "")) == "knock_fly" and p1 != null and p1.has_method("force_original_hurttype1_knock_fly_if_needed"):
		p1.call("force_original_hurttype1_knock_fly_if_needed", hit_vo, get_node_facing(p2))
	if str(hit_result.get("result", "")) != "ignored":
		p2_last_hit_vo_id = hit_vo_id
		p2_last_hit_action = action_name
		p2_test_attack_has_hit = true
	if str(hit_result.get("result", "")) == "catch":
		original_catch_target = p1
		if p1 != null and p1.has_method("set_original_caught_by_throw"):
			p1.set_original_caught_by_throw(true)
...
func check_active_hit_target() -> void:
	...
	var hit_vo: Dictionary = get_hit_vo_for_attacker(p1, current_hit_vo_id)
	...
	if p2 != null and p2.has_method("apply_original_hit"):
		hit_result = p2.apply_original_hit(hit_vo, get_node_facing(p1))
	if str(hit_result.get("result", "")) == "knock_fly" and p2 != null and p2.has_method("force_original_hurttype1_knock_fly_if_needed"):
		p2.call("force_original_hurttype1_knock_fly_if_needed", hit_vo, get_node_facing(p1))
	if str(hit_result.get("result", "")) != "ignored":
...
func try_use_p1_energy(cost: float, allow_overflow: bool, label_text: String) -> bool:
	if p1 == null or not p1.has_method("has_energy") or not p1.has_method("use_energy"):
		return true
	if as_bool(p1.call("has_energy", cost, allow_overflow)):
...
func try_use_p2_qi(cost: float, label_text: String) -> bool:
	if p2 == null:
		return true
	var current_qi: float = get_node_float_property(p2, "qi", 0.0)
	if current_qi + 0.01 >= cost:
		set_node_float_property(p2, "qi", maxf(0.0, current_qi - cost))
		print("原版必杀气消耗：", label_text, " cost=", cost, " qi=", get_node_float_property(p2, "qi", 0.0))
		return true
	if p2.has_method("use_qi") and p2.call("use_qi", cost):
		print("原版必杀气消耗：", label_text, " cost=", cost, " qi=", get_node_float_property(p2, "qi", 0.0))
		return true
	print("P2 气不足，不能释放 ", label_text, "；需要 ", cost, "，当前 qi=", get_node_float_property(p2, "qi", 0.0), "。按 P 可临时充满气用于测试。")
	return false


func try_use_p2_energy(cost: float, allow_overflow: bool, label_text: String) -> bool:
	if p2 == null or not p2.has_method("has_energy") or not p2.has_method("use_energy"):
		return true
	if as_bool(p2.call("has_energy", cost, allow_overflow)):
		p2.call("use_energy", cost)
		print("原版体力消耗：", label_text, " cost=", cost, " energy=", get_node_float_property(p2, "energy", 0.0))
		return true
	print("P2 体力不足，不能执行 ", label_text, "；需要 ", cost, "，当前 energy=", get_node_float_property(p2, "energy", 0.0))
	return false
...
func apply_original_effect_vo_feedback(meta: Dictionary, _hit_vo: Dictionary = {}) -> void:
	# 对照原版 EffectCtrl.doEffectVO：播放 EffectView 后统一处理 freeze / shake / shine。
	# shake.pow 在 doHitEffect / doDefenseEffect 中转换为 y 轴震动；break_def 使用显式 x 轴震动。
	var freeze_time: float = float(meta.get("freeze", 0.0))
	if freeze_time > 0.0:
		hit_stop_timer = maxf(hit_stop_timer, freeze_time)

	var shake_time: float = float(meta.get("shake_time", 0.0))
	var shake_x: float = float(meta.get("shake_x", 0.0))
	var shake_y: float = float(meta.get("shake_y", 0.0))
	var shake_pow: float = float(meta.get("shake_pow", 0.0))
	if shake_pow != 0.0:
		shake_x = 0.0
		shake_y = shake_pow
	if shake_time > 0.0 and (shake_x != 0.0 or shake_y != 0.0):
		start_original_shake(shake_x, shake_y, shake_time)

	var effect_shine_alpha: float = float(meta.get("shine_alpha", 0.0))
	if effect_shine_alpha > 0.0:
		var shine_color: Color = meta.get("shine_color", Color(1, 1, 1, 1)) as Color
		flash_shine(Color(shine_color.r, shine_color.g, shine_color.b, effect_shine_alpha), maxf(0.10, freeze_time * 0.35))


func should_skip_common_hit_sprite_for_bisha(_hit_vo: Dictionary) -> bool:
	# Step81：原版 EffectCtrl.doHitEffect 对 bs/bs1/sbs/cbs2/cbs/kbs 也会正常创建 hit_* EffectView。
	# Step55 曾为了避开当前 PNG+ADD 伪影跳过 bisha common sprite；这不是一比一。
	# 现在 CommonEffectLayer 已改为 alpha 预乘 ADD shader + 同帧上限，故恢复原版：不再跳过。
	return false
...
func after_p1_hit_target(hit_vo: Dictionary, hit_result: Dictionary, original_hit_rect: Rect2 = Rect2()) -> void:
	...
	play_original_hit_feedback(hit_vo, feedback_rect, result, get_node_facing(p1), p2)
...
func after_p2_hit_target(hit_vo: Dictionary, hit_result: Dictionary, original_hit_rect: Rect2 = Rect2()) -> void:
	...
	var hit_source: String = p2_active_original_hitbox if p2_active_original_hitbox != "" else "main_attack"
	if result != "ignored":
		add_qi_on_hit(p2, p1, hit_vo, hit_source)
...
func play_original_hit_feedback(hit_vo: Dictionary, hit_rect: Rect2, result: String, facing: int, target_fighter = null) -> void:
	...
	var effect_hit_rect: Rect2 = get_original_swf_effect_hit_rect(hit_rect, hit_vo)
	var hit_type: int = int(hit_vo.get("hitType", 0))

	if result == "defense":
		# 原版 doDefenseEffect -> EffectModel.getDefenseEffect(hitType, defenseType) -> doEffectVO。
		# 这里按 EffectModel.as 的 defense_* freeze/shine/shake 完整补齐。
		if common_effects != null:
			common_effects.play_defense_effect(hit_vo, effect_hit_rect, facing)
		var def_meta: Dictionary = ORIGINAL_DEFENSE_EFFECT_META.get(hit_type, ORIGINAL_DEFENSE_EFFECT_META.get(5, {})) as Dictionary
		apply_original_effect_vo_feedback(def_meta, hit_vo)
		print("原版 doDefenseEffect: hitType=", hit_type, " rect=", hit_rect, " meta=", def_meta)
		return

	if result == "steel":
		# 原版 doSteelHitEffect 使用 steel_hit_kan / steel_hit_qdj / steel_hit_mfdj，freeze=400，shine alpha=0.2，sound=snd_hit11。
		...
		apply_original_effect_vo_feedback(ORIGINAL_STEEL_HIT_META, hit_vo)
		print("原版 doSteelHitEffect: hitType=", hit_type, " rect=", hit_rect)
		return
	...
	if result == "catch":
		...
		var catch_meta: Dictionary = ORIGINAL_HIT_EFFECT_META.get(11, {}) as Dictionary
		apply_original_effect_vo_feedback(catch_meta, hit_vo)
		print("原版 catch hit: hitType=", hit_type, " rect=", hit_rect)
		return

	if result == "break_defense":
		# EffectModel.break_def: XG_mfdjx + snd_mfdjx + slowDown(1.5,1500) + freeze=500 + shake{x:6,time:400}。
		if common_effects != null:
			common_effects.play_break_defense(effect_hit_rect, facing)
		apply_original_effect_vo_feedback(ORIGINAL_BREAK_DEF_META, hit_vo)
		play_original_slow_down_event("break_defense", -1, [1.5, 1500])
	...
	if common_effects != null and not suppress_bisha_common_sprite:
		...
		# 保留原版 EffectVO 的声音/freeze/shine/shake；仅跳过当前预渲染 common hit sprite，避免 bisha 多段叠成电视雪花。
		play_original_hit_sound_only(hit_vo)
		print("Step81 skip bisha common sprite only; keep EffectVO feedback HitVO=", str(hit_vo.get("id", "")), " rect=", hit_rect)

	var hit_meta: Dictionary = ORIGINAL_HIT_EFFECT_META.get(hit_type, {}) as Dictionary
	apply_original_effect_vo_feedback(hit_meta, hit_vo)

	if hit_type == 9:
		# EffectModel.as: ELECTRIC -> specialEffectId shock_ing + targetColorOffset [50,-75,255]。
...
func begin_original_bisha_super_camera_focus(caster: Variant) -> void:
	# 原版 EffectCtrl.bisha(isSuper=true)：GameStage.cameraFocusOne(target.getDisplay())。
	# 这里 immediate=true 是为了在 HUD 层播放 XG_cbs 前完成 world_to_screen 换算；
	# 否则 XG_cbs 会按旧双人镜头坐标生成，和原版特写位置不一致。
	if caster == null:
		return
	if not is_instance_valid(caster):
		return
	original_camera_focus_one(caster, true)
	original_bisha_super_camera_focus_active = true
...
func begin_original_bisha_stage_mode(is_super: bool) -> void:
	...
	if is_super:
		# cameraFocusOne 已在 handle_aizen*_semantic_event 中按施放者执行；这里不再重复。
		print("Step80 bisha_super cameraFocusOne active=", original_bisha_super_camera_focus_active)
...
func end_original_bisha_stage_mode() -> void:
	...
	for state in original_bisha_saved_canvas_states:
		...
		if not (node is CanvasItem):
			continue
		var item := node as CanvasItem
		item.visible = as_bool(state.get("visible", true))
	...
	if original_bisha_super_camera_focus_active:
		# 原版 EffectCtrl.endBisha(): GameStage.cameraResume()。
		original_camera_resume()
		update_original_camera(0.0, false)
		original_bisha_super_camera_focus_active = false


func force_hide_legacy_blackout_layers_recursive(node: Node) -> void:
	if node == null:
		return
	if node.has_meta("keep_round_end_ui") and as_bool(node.get_meta("keep_round_end_ui")):
		return
	if node.has_meta("keep_background_backfill") and as_bool(node.get_meta("keep_background_backfill")):
		return
...
func is_original_loser_dead_ready() -> bool:
	var loser = get_original_round_loser()
	if loser == null:
		return true
	if loser.has_method("is_original_round_end_dead_down"):
		return as_bool(loser.call("is_original_round_end_dead_down"))
	return true


func get_original_round_winner() -> Variant:
	if ko_winner_id == 1:
		return p1
	if ko_winner_id == 2:
		return p2
...
func get_original_round_loser() -> Variant:
	if ko_winner_id == 1:
		return p2
	if ko_winner_id == 2:
		return p1
...
func apply_original_round_end_actions(reason: String) -> void:
	# 原版 GameEndCtrl/FightUI 流程：KO 后先锁输入与伤害，败者进入击飞/倒地/DEAD；
	# 等 KO UI 与败者倒地结束后，cameraFocusOne(winner)，然后 GameEndCtrler 调 winner.win()。
	# 因此这里不能像旧 Godot 补丁那样立即让胜者 win、败者 lose。
	if original_round_end_actions_applied:
		return
	original_round_end_actions_applied = true
	clear_p1_input_buffers()
	clear_p2_input_buffers()
	clear_original_timeline_sfx_events()
	...
	var loser = get_original_round_loser()
	var winner = get_original_round_winner()
	if reason == "ko" and loser != null:
		# FighterMain.die()：如果 KO 前已经在受击/击飞，则继续原本击飞流程；
		# renderHurtFly state4 检测到 !isAlive 后进入 DEAD 并停在倒地。
		# Step70：把最后一击 HitVO 也传入 ManifestFighter，避免 Godot 侧 KO 检测晚一帧时丢失原版 hitx/hity/hurtType。
		var ko_attacker_facing: int = get_node_facing(winner) if winner != null else (1 if ko_winner_id == 1 else -1)
		var last_hit_id: String = p1_last_hit_vo_id if ko_winner_id == 1 else p2_last_hit_vo_id
		var last_hit_vo: Dictionary = AIZEN_HIT_VO.get(last_hit_id, {})
		if loser.has_method("play_original_round_end_loser_down"):
			loser.call("play_original_round_end_loser_down", ko_attacker_facing, last_hit_vo)
		elif loser.has_method("play_action"):
			loser.play_action("knock_down")
	print("原版 round end state: reason=", reason, " winner=", ko_winner_id, " loser fly/down->DEAD; winner win action delayed until camera focus.")
...
func check_ko_state() -> void:
	...
	if ko_loser != null and common_effects != null and common_effects.has_method("play_ko_hit_end"):
		var ko_loser_fallback: Vector2 = P1_START_POS if ko_loser == p1 else P2_START_POS
		common_effects.play_ko_hit_end(get_original_swf_effect_origin(ko_loser, ko_loser_fallback), get_node_facing(ko_loser))
...
func handle_original_move_target_event(action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	# P1 兼容包装：原版 FighterMcCtrler.moveTarget(params) 绑定 _fighter.getCurrentTarget()。
	handle_original_move_target_event_for_actor(p1, p2, action_name, relative_frame, semantic, "P1")


func handle_original_move_target_event_for_actor(actor: Variant, target: Variant, action_name: String, relative_frame: int, semantic: Dictionary, actor_label: String = "") -> void:
	# FighterMcCtrler.moveTarget(params): params=null 时关闭；否则创建 MoveTargetParamVO 并绑定施法者当前目标。
	# 原版依据：FighterMcCtrler.moveTarget -> new MoveTargetParamVO(params, target)；target 来自当前 fighter 的 currentTarget。
	var args_raw: String = str(semantic.get("args_raw", "")).strip_edges()
	if args_raw == "" or args_raw.to_lower() == "null":
		clear_original_move_target_binding("moveTarget(null) actor=" + actor_label + " action=" + action_name + " frame=" + str(relative_frame))
		return

	if actor == null or target == null or not is_instance_valid(actor) or not is_instance_valid(target):
		clear_original_move_target_binding("moveTarget actor/target missing actor=" + actor_label + " action=" + action_name)
		return

	var parsed: Dictionary = parse_original_move_target_params(args_raw)
	# 全局一次只允许一个 moveTarget；新事件覆盖旧事件前先释放旧目标，避免 P2 被错误锁定。
	if original_move_target_active or original_move_target_target != null:
		clear_original_move_target_binding("moveTarget replaced by actor=" + actor_label)

	original_move_target_active = true
	original_move_target_actor = actor
	original_move_target_target = target
	...
	print("原版 moveTarget 开启 actor=", actor_label, " action=", action_name, " frame=", relative_frame, " followmc=", original_move_target_follow_mc_name, " fixed=", original_move_target_fixed_pos, " hasXY=", Vector2(1 if original_move_target_has_fixed_x else 0, 1 if original_move_target_has_fixed_y else 0), " speed/frame=", original_move_target_speed_per_frame)
...
func update_original_move_target(delta: float) -> void:
	if not original_move_target_active:
		return
	if original_move_target_actor == null or original_move_target_target == null or not is_instance_valid(original_move_target_actor) or not is_instance_valid(original_move_target_target):
		clear_original_move_target_binding("actor/target missing")
		return
...
func get_original_move_target_aim() -> Vector2:
	if original_move_target_actor == null or not is_instance_valid(original_move_target_actor):
		return Vector2.ZERO
	var actor_node := original_move_target_actor as Node2D
	if actor_node == null:
		return Vector2.ZERO
	if original_move_target_follow_mc_name == "move_mc":
		var actor_facing: int = int(actor_node.get("facing"))
		if actor_facing == 0:
			actor_facing = 1
		return actor_node.position + Vector2(ORIGINAL_THROW2_MOVE_MC_LOCAL_POS.x * float(actor_facing), ORIGINAL_THROW2_MOVE_MC_LOCAL_POS.y)
...
func clear_original_move_target_binding(reason: String = "") -> void:
	...
	original_move_target_actor = null
...
func is_original_world_position_locked(node) -> bool:
	...
	if node.has_method("is_original_world_position_locked"):
		return as_bool(node.is_original_world_position_locked())
	var locked_value: Variant = node.get("original_no_world_move_anchor_active")
	if locked_value != null:
		return as_bool(locked_value)
	return false


func get_original_fighter_body_rect(node: Variant) -> Rect2:
	# 原版 FighterCtrler.getBodyArea() 来自当前帧名为 bdmn 的显示对象；
	# Aizen 当前 bdmn 采用 BDMN_DEFAULT_HURT_RECT 这一套矩形。
	if node == null or not is_instance_valid(node):
		return Rect2()
	if not (node is Node2D):
		return Rect2()
	var ground_pos: Vector2 = get_node_ground_pos(node, Vector2.ZERO)
	var direct: int = get_node_facing(node)
	return make_directed_rect(ground_pos, BDMN_DEFAULT_HURT_RECT, direct)


func original_move_fighter_x(node: Variant, dx: float) -> void:
	if node == null or not is_instance_valid(node):
		return
	if not (node is Node2D):
		return
	(node as Node2D).position.x += dx
	clamp_fighter_to_original_map_bounds(node)


func resolve_original_cross_collision() -> void:
	# 对照原版 GameMainLogicCtrler.checkBodyHit(A,B)：
	# 1) 任一方 isCross=true 时允许穿身；
	# 2) 用双方 getBodyArea() 的交集判断是否重叠；
	# 3) 重叠宽度 > 2 时，以 5 * SPEED_PLUS 做基础分离；
	# 4) 再按双方当前 vecX，把“推人”的位移按 heavy 比例分给双方；
	# 5) 碰到地图/镜头边界的一方 getIsTouchSide=true，则把位移更多分给另一方。
	if p1 == null or p2 == null:
		return
	...
	if original_move_target_active:
		return
	var p1_cross: bool = as_bool(p1.get("is_cross")) if p1.get("is_cross") != null else false
	var p2_cross: bool = as_bool(p2.get("is_cross")) if p2.get("is_cross") != null else false
	if p1_cross or p2_cross:
		return

	# 保留早期修复：原版无根移动动作在 Godot 中用世界坐标锁定补偿，不能再被身体分离推开。
	if is_original_world_position_locked(p1) or is_original_world_position_locked(p2):
		return

	var body_a: Rect2 = get_original_fighter_body_rect(p1)
	var body_b: Rect2 = get_original_fighter_body_rect(p2)
	if not body_a.has_area() or not body_b.has_area():
		return
	var body_hit: Rect2 = body_a.intersection(body_b)
	if not body_hit.has_area():
		return

	var n1 := p1 as Node2D
	var n2 := p2 as Node2D
	var vec_a: float = get_original_node_vec_x(p1) / ORIGINAL_GAME_RENDER_FPS
	var vec_b: float = get_original_node_vec_x(p2) / ORIGINAL_GAME_RENDER_FPS

	var base_speed: float = 5.0 * ORIGINAL_SPEED_PLUS_DEFAULT
	var over_vec: Dictionary = {}

	if n1.position.x < n2.position.x:
		if (vec_a < 0.0 and vec_a < vec_b) or (vec_b > 0.0 and vec_b > vec_a):
			return
		if body_hit.size.x > 2.0:
			over_vec = get_original_body_push_vec(base_speed)
			original_move_fighter_x(p1, -float(over_vec.get("A", 0.0)))
			original_move_fighter_x(p2, float(over_vec.get("B", 0.0)))
	else:
		if (vec_a > 0.0 and vec_a > vec_b) or (vec_b < 0.0 and vec_b < vec_a):
			return
		if body_hit.size.x > 2.0:
	...
	if vec_a != 0.0:
		var vo_a: Dictionary = get_original_body_push_vec(vec_a)
		original_move_fighter_x(p2, float(vo_a.get("B", 0.0)))
		original_move_fighter_x(p1, -float(vo_a.get("A", 0.0)))

	if vec_b != 0.0:
		var vo_b: Dictionary = get_original_body_push_vec(vec_b)
		original_move_fighter_x(p1, float(vo_b.get("A", 0.0)))
		original_move_fighter_x(p2, -float(vo_b.get("B", 0.0)))


func get_original_body_push_vec(vec: float) -> Dictionary:
	# 原版 heavy 默认双方均为 2，所以 rate = 0.5；保留 0.1~0.9 clamp。
	var heavy_a: float = 2.0
	var heavy_b: float = 2.0
	if p1 != null and p1.get("heavy") != null:
		var hv_a: Variant = p1.get("heavy")
		if typeof(hv_a) == TYPE_INT or typeof(hv_a) == TYPE_FLOAT:
			heavy_a = maxf(0.001, float(hv_a))
	if p2 != null and p2.get("heavy") != null:
		var hv_b: Variant = p2.get("heavy")
		if typeof(hv_b) == TYPE_INT or typeof(hv_b) == TYPE_FLOAT:
			heavy_b = maxf(0.001, float(hv_b))

	var rate: float = heavy_b / heavy_a / 2.0
	rate = clampf(rate, 0.1, 0.9)
	var vec_a: float = vec * rate
	var vec_b: float = vec * (1.0 - rate)

	var a_touch: bool = get_original_touch_side(p1)
	var b_touch: bool = get_original_touch_side(p2)
...
func is_original_target_hurt_ing(target) -> bool:
	...
	if action_name == "hurt" or action_name.begins_with("knock"):
		return true
	if target.get("in_hitstun") != null and as_bool(target.get("in_hitstun")):
		return true
...
func is_original_true_catch_direction() -> bool:
	if p1 == null or p2 == null:
		return false
	var self_left_of_target: bool = get_node_ground_pos(p1, P1_START_POS).x < get_node_ground_pos(p2, P2_START_POS).x
	var move_right: bool = Input.is_key_pressed(KEY_D)
	var move_left: bool = Input.is_key_pressed(KEY_A)
	return (move_right and self_left_of_target and get_node_facing(p1) == 1) or (move_left and (not self_left_of_target) and get_node_facing(p1) == -1)
...
func allow_original_catch() -> bool:
	...
	var dis_y: float = absf(get_node_ground_pos(p1, P1_START_POS).y - get_node_ground_pos(p2, P2_START_POS).y)
...
func start_original_catch_action(action_name: String, catch_name: String) -> void:
	...
	if p1.has_method("set_facing"):
		node_set_facing_safe(p1, 1 if get_node_ground_pos(p2, P2_START_POS).x >= get_node_ground_pos(p1, P1_START_POS).x else -1)
	node_play_action_safe(p1, action_name)
	print("原版 doCatch 触发：", catch_name, " -> ", action_name, " target=", p2, " p1=", get_node_ground_pos(p1, P1_START_POS), " p2=", get_node_ground_pos(p2, P2_START_POS))
...
func play_character_timeline_sound(fighter: Variant, semantic: Dictionary, action_name: String, relative_frame: int) -> void:
	var char_id: String = get_fighter_character_id(fighter, "aizen")
	var sound_id: String = str(semantic.get("sound_id", ""))
	if sound_id == "":
		return

	# Step118：优先使用 manifest 中从原版 DOMSoundItem 推出的 sound_path。
	var sound_path: String = str(semantic.get("sound_path", ""))
	if sound_path != "" and (ResourceLoader.exists(sound_path) or FileAccess.file_exists(sound_path)):
		AudioManager.play_sfx(sound_path, -2.0)
		print("原版角色时间轴 soundName 触发 path=", sound_path, " char=", char_id, " action=", action_name, " frame=", relative_frame)
		return

	if AudioManager != null and AudioManager.has_method("play_character_sfx"):
		AudioManager.call("play_character_sfx", char_id, sound_id, -2.0)
	else:
		var fallback_path: String = "res://assets/sfx/" + char_id + "/" + sound_id + ".mp3"
		if ResourceLoader.exists(fallback_path) or FileAccess.file_exists(fallback_path):
			AudioManager.play_sfx(fallback_path, -2.0)
	print("原版角色时间轴 soundName 触发 char=", char_id, " action=", action_name, " frame=", relative_frame, " sound=", sound_id)
...
func apply_original_slow_runtime() -> void:
	...
	_set_original_time_scale_on_node(p2_effect, slow_scale)
...
func move_fighter_once_by_original_args(fighter: Variant, action_name: String, relative_frame: int, args: Array) -> void:
	...
	var direct: int = get_node_facing(fighter)
	fighter.position.x += dx * float(direct)
	fighter.position.y += dy
	clamp_fighter_to_original_map_bounds(fighter)
	if fighter.has_method("set_air_state_from_position"):
		fighter.set_air_state_from_position()
	var fighter_label: String = str(fighter.name) if fighter is Node else "?"
	print("原版 moveOnce 触发 action=", action_name, " frame=", relative_frame, " fighter=", fighter_label, " delta=", Vector2(dx, dy), " new_pos=", fighter.position)


func move_p1_once_by_original_args(action_name: String, relative_frame: int, args: Array) -> void:
	move_fighter_once_by_original_args(p1, action_name, relative_frame, args)


func face_fighter_to_target(fighter: Variant, target: Variant, action_name: String, relative_frame: int, label_text: String) -> void:
	# FighterCtrler.setDirectToTarget(): fighter.direct = fighter.x > target.x ? -1 : 1。
	if fighter == null or target == null:
		return
	if fighter.has_method("set_facing"):
		var fighter_pos: Vector2 = get_node_ground_pos(fighter, P1_START_POS)
		var target_pos: Vector2 = get_node_ground_pos(target, P2_START_POS)
		fighter.set_facing(-1 if fighter_pos.x > target_pos.x else 1)
	print("原版 setDirectToTarget 触发 ", label_text, " action=", action_name, " frame=", relative_frame, " facing=", get_node_facing(fighter))


func face_p1_current_target(action_name: String, relative_frame: int) -> void:
	face_fighter_to_target(p1, p2, action_name, relative_frame, "P1")


func face_p2_current_target(action_name: String, relative_frame: int) -> void:
	face_fighter_to_target(p2, p1, action_name, relative_frame, "P2")


func set_fighter_cross_enabled(fighter: Variant, enabled: bool, action_name: String, relative_frame: int, label_text: String) -> void:
	# FighterCtrler.setCross(v): _fighter.isCross = v
	if fighter == null:
		return
	if fighter.has_method("set_cross_enabled"):
		fighter.set_cross_enabled(enabled)
	else:
		fighter.set_meta("original_is_cross", enabled)
	print("原版 setCross 触发 ", label_text, " action=", action_name, " frame=", relative_frame, " enabled=", enabled)


func set_p1_cross_enabled(enabled: bool, action_name: String, relative_frame: int) -> void:
	set_fighter_cross_enabled(p1, enabled, action_name, relative_frame, "P1")


func handle_original_just_hit_to_play_for_fighter(fighter: Variant, is_p2_actor: bool, action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	...
	var last_id: String = p2_last_hit_vo_id if is_p2_actor else p1_last_hit_vo_id
	var last_action: String = p2_last_hit_action if is_p2_actor else p1_last_hit_action
	var matched: bool = attack_id != "" and last_id == attack_id and last_action == action_name
	print("原版 justHitToPlay 检查 actor=", "P2" if is_p2_actor else "P1", " action=", action_name, " frame=", relative_frame, " attack_id=", attack_id, " last=", last_id, "/", last_action, " matched=", matched, " target=", target_action)

	if matched and target_action != "" and fighter != null:
		fighter.play_action(target_action)
		if is_p2_actor:
			p2_last_hit_vo_id = ""
			p2_last_hit_action = ""
		else:
			p1_last_hit_vo_id = ""
			p1_last_hit_action = ""
	elif return_idle_on_fail and fighter != null and fighter.has_method("return_to_neutral_from_timeline"):
		fighter.return_to_neutral_from_timeline()


func handle_original_just_hit_to_play(action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	handle_original_just_hit_to_play_for_fighter(p1, false, action_name, relative_frame, semantic)


func move_fighter_to_target(fighter: Variant, target: Variant, offset_x_value: Variant, offset_y_value: Variant, auto_turn: bool) -> void:
	if fighter == null or target == null:
		return
	var target_pos: Vector2 = get_node_ground_pos(target, P2_START_POS)
	...
	if has_x:
		var fx: float = target_pos.x + offset_x * float(direct)
		if offset_x > 0.0:
			if target_pos.x < offset_x:
				fx = target_pos.x + offset_x
			elif target_pos.x > ORIGINAL_MAP_RIGHT - offset_x:
				fx = target_pos.x - offset_x
		fighter.position.x = fx

	if has_y:
		fighter.position.y = target_pos.y + offset_y

	clamp_fighter_to_original_map_bounds(fighter)
	if fighter.has_method("set_air_state_from_position"):
		fighter.set_air_state_from_position()
	if auto_turn and fighter.has_method("set_facing"):
		var new_direct: int = 1 if fighter.position.x < target_pos.x else -1
		fighter.set_facing(new_direct)
	var fighter_label: String = str(fighter.name) if fighter is Node else "?"
	print("原版 moveToTarget 触发 fighter=", fighter_label, " x=", offset_x_value, " y=", offset_y_value, " autoTurn=", auto_turn, " new_pos=", fighter.position)


func move_p1_to_current_target(offset_x_value: Variant, offset_y_value: Variant, auto_turn: bool) -> void:
	move_fighter_to_target(p1, p2, offset_x_value, offset_y_value, auto_turn)
...
func play_original_dash_effect(action_name: String, relative_frame: int) -> void:
	...
	var is_air: bool = p1 != null and as_bool(p1.get("in_air"))
	...
	if common_effects != null and common_effects.has_method("play_dash_effect"):
		common_effects.play_dash_effect(dash_effect_origin, get_node_facing(p1), is_air)
		print("原版 dash effect 触发 action=", action_name, " frame=", relative_frame, " effect=", effect_id)
	elif _common_effect_exists(effect_id):
		common_effects.play_effect(effect_id, dash_effect_origin, get_node_facing(p1), 1.0, false)
		AudioManager.play_effect_sfx("snd_dash_air")
		print("原版 dash effect 触发 action=", action_name, " frame=", relative_frame, " effect=", effect_id)
	else:
		print("原版 dash effect 触发但资源未接入：", effect_id, " action=", action_name, " frame=", relative_frame)

func play_original_dash_effect_for_fighter(fighter: Variant, action_name: String, relative_frame: int) -> void:
	if fighter == null:
		return
	var is_air: bool = as_bool(fighter.get("in_air")) if fighter != null else false
	var dash_effect_origin: Vector2 = get_original_dash_dust_origin_for_fighter(fighter, P2_START_POS)
	var fighter_facing: int = get_node_facing(fighter)
	if common_effects != null and common_effects.has_method("play_dash_effect"):
		common_effects.play_dash_effect(dash_effect_origin, fighter_facing, is_air)
		print("原版 P2 dash effect 触发 action=", action_name, " frame=", relative_frame)
	elif _common_effect_exists("dash_air" if is_air else "dash"):
		common_effects.play_effect("dash_air" if is_air else "dash", dash_effect_origin, fighter_facing, 1.0, false)
		AudioManager.play_effect_sfx("snd_dash_air")
...
func handle_original_movement_event_for_fighter(fighter: Variant, action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	var method_name: String = str(semantic.get("method", ""))
	var args: Array = semantic.get("args", []) as Array
	match method_name:
		"move":
			var vx: float = _original_args_number(args, 0, 0.0) * float(get_node_facing(fighter)) * 30.0
			var vy: float = _original_args_number(args, 1, 0.0) * 30.0
			_apply_original_velocity_for_fighter(fighter, vx, vy)
		"movePercent":
			var vx_percent: float = move_speed * _original_args_number(args, 0, 0.0) * float(get_node_facing(fighter))
			var vy_percent: float = move_speed * _original_args_number(args, 1, 0.0)
			_apply_original_velocity_for_fighter(fighter, vx_percent, vy_percent)
		"damping":
			var damping_x: float = absf(_original_args_number(args, 0, 0.0)) * 30.0 * 30.0
			_apply_original_damping_for_fighter(fighter, damping_x)
		"dampingPercent":
			var damping_percent_x: float = absf(move_speed * _original_args_number(args, 0, 0.0)) * 30.0
			_apply_original_damping_for_fighter(fighter, damping_percent_x)
		_:
			print("原版 movement 事件暂未接入 actor generic action=", action_name, " frame=", relative_frame, " data=", semantic)
...
func handle_original_movement_event(action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	...
	match method_name:
		"move":
			...
			var vx: float = _original_args_number(args, 0, 0.0) * float(get_node_facing(p1)) * 30.0
			var vy: float = _original_args_number(args, 1, 0.0) * 30.0
		...
		"movePercent":
			...
			var vx_percent: float = move_speed * _original_args_number(args, 0, 0.0) * float(get_node_facing(p1))
			var vy_percent: float = move_speed * _original_args_number(args, 1, 0.0)
...
func handle_original_dash_or_stop_event_for_fighter(fighter: Variant, action_name: String, relative_frame: int, semantic: Dictionary) -> void:
	var method_name: String = str(semantic.get("method", ""))
	var args: Array = semantic.get("args", []) as Array
	match method_name:
		"dash":
			var speed_plus: float = _original_args_number(args, 0, 3.0)
			if fighter != null and fighter.has_method("apply_original_dash_motion"):
				fighter.apply_original_dash_motion(speed_plus, move_speed)
		"dashStop":
			var lose_percent: float = _original_args_number(args, 0, 0.5)
			if fighter != null and fighter.has_method("apply_original_dash_stop"):
				fighter.apply_original_dash_stop(lose_percent)
		"stopMove":
			if fighter != null and fighter.has_method("stop_original_motion"):
				fighter.stop_original_motion()
		_:
			print("原版 dash_or_stop 事件暂未接入 actor generic action=", action_name, " frame=", relative_frame, " data=", semantic)
...
func _apply_original_velocity_for_fighter(fighter: Variant, vx: float, vy: float) -> void:
	if fighter == null:
		return
	if fighter.has_method("set_original_velocity"):
		fighter.set_original_velocity(vx, vy)


func _apply_original_damping_for_fighter(fighter: Variant, damping_x: float) -> void:
	if fighter == null:
		return
	if fighter.has_method("set_original_damping"):
		fighter.set_original_damping(damping_x)
...
func get_p2_target_original_effect_origin() -> Vector2:
	# 2P 接入后，P2 的 currentTarget 固定为 P1。
	return get_p1_original_effect_origin()


func get_original_swf_effect_hit_rect(hit_rect: Rect2, hit_vo: Dictionary = {}) -> Rect2:
	# 只修正视觉特效高度；不改变 current_attack_world_rect / 碰撞判定。
	# 原版 EffectCtrl.doHitEffect(hitvo, hitRect) 的落点是 hitRect.center；
	# 这里根据 HitVO id 选择显示层注册点修正，避免 J/连续J 刀击特效压到脚边。
	var y_correction: float = ORIGINAL_HIT_EFFECT_VISUAL_Y_CORRECTION
	var hit_id: String = str(hit_vo.get("id", ""))
	if hit_id in ORIGINAL_AIZEN_J_HIT_IDS:
		y_correction = ORIGINAL_AIZEN_J_HIT_EFFECT_VISUAL_Y_CORRECTION
	return Rect2(hit_rect.position + Vector2(0.0, y_correction), hit_rect.size)
...
func play_p1_effect(effect_name: String, offset: Vector2, effect_scale: float = 1.0) -> void:
	...
	p1_effect.play_effect(effect_name, pos, get_node_facing(p1), effect_scale)
...
func play_p1_attacker_at_target(effect_name: String, target_offset: Vector2, effect_scale: float = 1.0) -> void:
	...
	active_attacker_facing = get_node_facing(p1)
	active_attacker_has_hit = false
...
func get_p2_target_ground_pos() -> Vector2:
	# P2 currentTarget 固定为 P1。
	return get_node_ground_pos(p1, P1_START_POS)


func _reset_p2_active_attacker_state() -> void:
	p2_active_attacker_follow_target = false
	p2_active_attacker_effect_name = ""
	p2_active_attacker_offset = Vector2.ZERO
	p2_active_attacker_original_origin = Vector2.ZERO
	p2_active_attacker_facing = -1
	p2_active_attacker_start_ms = 0
	p2_test_attack_has_hit = false
	p2_test_attack_key = ""


func _p2_attacker_visual_position_from_origin() -> Vector2:
	return p2_active_attacker_original_origin


func play_p2_attacker_at_target(effect_name: String, target_offset: Vector2, effect_scale: float = 1.0) -> void:
	if p2 == null or p2_effect == null:
		return
	p2_active_attacker_follow_target = true
	p2_active_attacker_effect_name = effect_name
	p2_active_attacker_offset = target_offset
	var target_origin: Vector2 = get_p2_target_original_effect_origin()
	p2_active_attacker_original_origin = align_original_skill_effect_height(effect_name, target_origin + target_offset, target_origin, target_offset)
	p2_active_attacker_facing = get_node_facing(p2)
	p2_test_attack_has_hit = false
	p2_effect.play_effect(effect_name, _p2_attacker_visual_position_from_origin(), p2_active_attacker_facing, effect_scale)


func play_p2_attacker_at_self(effect_name: String, self_offset: Vector2, effect_scale: float = 1.0) -> void:
	if p2 == null or p2_effect == null:
		return
	p2_active_attacker_follow_target = false
	p2_active_attacker_effect_name = effect_name
	p2_active_attacker_offset = self_offset
	var caster_origin: Vector2 = get_p2_original_effect_origin()
	p2_active_attacker_original_origin = align_original_skill_effect_height(effect_name, caster_origin + self_offset, caster_origin, self_offset)
	p2_active_attacker_facing = get_node_facing(p2)
	p2_test_attack_has_hit = false
	p2_effect.play_effect(effect_name, p2_active_attacker_original_origin + self_offset, p2_active_attacker_facing, effect_scale, true)


func update_p2_attacker_follow_target() -> void:
	if not p2_active_attacker_follow_target:
		return
	if p2_effect == null:
		_reset_p2_active_attacker_state()
		return
	if not p2_effect.playing or p2_effect.current_effect != p2_active_attacker_effect_name:
		_reset_p2_active_attacker_state()
		return
	var target_origin: Vector2 = get_p2_target_original_effect_origin()
	p2_active_attacker_original_origin = align_original_skill_effect_height(p2_active_attacker_effect_name, target_origin + p2_active_attacker_offset, target_origin, p2_active_attacker_offset)
	p2_effect.position = _p2_attacker_visual_position_from_origin()
